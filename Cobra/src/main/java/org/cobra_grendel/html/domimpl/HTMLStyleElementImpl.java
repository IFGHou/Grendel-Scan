...
public class HTMLStyleElementImpl extends HTMLElementImpl implements HTMLStyleElement
{
	...
	public HTMLStyleElementImpl(int transactionId)
	{
		super("STYLE", true, transactionId);
	}
	
	public HTMLStyleElementImpl(String name, int transactionId)
	{
	...
	@Override public String getMedia()
	{
	 ...
	 * this.getHtmlRendererContext())) { String text =
	 * this.getRawInnerText(true); if(text != null && !"".equals(text)) { String
	 * processedText = CSSUtilities.preProcessCss(text); HTMLDocumentImpl doc =
	 * (HTMLDocumentImpl) this.getOwnerDocument(); // CSSOMParser parser = new
	 * CSSOMParser(); InputSource is =
	 * CSSUtilities.getCssInputSourceForStyleSheet(processedText); try {
	 * CSSStyleSheet sheet = parser.parseStyleSheet(is);
	 * doc.addStyleSheet(sheet); } catch(Throwable err) { this.warn("Unable to
	 * parse style sheet", err); } } } }
	 */
}
