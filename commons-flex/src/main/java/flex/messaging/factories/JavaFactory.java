...
/**
 * This class implements the FlexFactory interface to constructs Flex messaging components. The JavaFactory uses the class name, specified as the source attribute to determine the class for instances.
 * The scope attribute can be one of session, application or request to determine its lifecycle. If you use application or session, you can specify the optional attribute-id parameter to control the
 * name of the key for storing the component. Two destinations using the same attribute-id will use the same component. The component is stored in the ServletContext (for application scoped
 * components) and in the session (for session scoped components) so you can use these components in your JSP as well.
 * 
 * @author Jeff Vroom
 */
public class JavaFactory implements FlexFactory, DestructibleFlexFactory
{
    ...
    /**
     * This method is called when we initialize the definition of an instance which will be looked up by this factory. It should validate that the properties supplied are valid to define an instance.
            ...
            try
            {
                ...
                synchronized (ctx)
                {
                    ...
                    else
                    {
                        ...
                        if (configuredClass != instClass && !configuredClass.isAssignableFrom(instClass))
                        {
                            ServiceException e = new ServiceException();
                            e.setMessage(INVALID_CLASS_FOUND, new Object[] { instance.getAttributeId(), "application", instance.getId(), instance.getInstanceClass(), inst.getClass() });
    ...
    /**
     * Returns the instance specified by the source and properties arguments. For the factory, this may mean constructing a new instance, optionally registering it in some other name space such as the
     * session or JNDI, and then returning it or it may mean creating a new instance and returning it. This method is called for each request to operate on the given item by the system so it should be
        ...
        else if (factoryInstance.getScope().equalsIgnoreCase(SCOPE_SESSION))
        {
            ...
            if (session != null)
            {
                ...
                if (instance != null)
                {
                    ...
                    {
                        ServiceException e = new ServiceException();
                        e.setMessage(INVALID_CLASS_FOUND, new Object[] { factoryInstance.getAttributeId(), "session", factoryInstance.getId(), factoryInstance.getInstanceClass(), instance.getClass() });
                        e.setCode("Server.Processing");
                        throw e;
                    }
                }
    ...
    /**
     * This method is called when a component using this factory is being destroyed. When appropriate, it frees up resources that were used by the factory instance and are no longer needed
     * 
     * @param inst
     *            The FactoryInstance to be cleaned up
     */
    @Override
