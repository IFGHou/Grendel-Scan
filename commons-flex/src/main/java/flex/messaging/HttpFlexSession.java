...
public class HttpFlexSession extends FlexSession implements HttpSessionBindingListener, HttpSessionListener, HttpSessionAttributeListener, Serializable
{
    ...
    /**
     * @exclude Not for public use; create new instances using getFlexSession(...). This no-args constructor allows HttpFlexSession to be registered as a listener in web.xml so we can detect changes
     *          to J2EE HttpSession attributes and proxy attribute notifications to FlexSession attribute and binding listeners.
     */
    ...
    /**
     * This attribute is set on the request associated with a Flex Session when a logout command is being processed. The reason that this is necessary is that a single HttpServletRequest may contain
     * more than one Flex command/message. In this case, every message following a "logout" command should behave as if the user has logged out. However, since in getUserPrincipal, we check the
     * request object if the Session has no principal, if the current request object has a principal object associated with it (as it does on Tomcat/JBoss), messages following a "logout" will use this
     * principal. We thus need to invalidate the user principal in the request on logout. Since the field is read-only we do so using this attribute.
     */
    ...
    /**
     * Flag to indicate whether we've logged a warning if we weren't registered in web.xml and can't redispatch attribute and binding events to Flex listeners.
     */
     ...
     * 
     * NOTE: Callback is not made against an HttpFlexSession associated with a request handling thread.
     */
    @Override
    ...
    public static HttpFlexSession getFlexSession(HttpServletRequest req)
    {
        ...
        if (!isHttpSessionListener && !warnedNoEventRedispatch)
        {
            ...
            if (Log.isWarn())
                Log.getLogger(WARN_LOG_CATEGORY)
                                .warn("HttpFlexSession has not been registered as a listener in web.xml for this application so no events will be dispatched to FlexSessionAttributeListeners or FlexSessionBindingListeners. To correct this, register flex.messaging.HttpFlexSession as a listener in web.xml.");
        }
    ...
    /**
     * @exclude Used by Http endpoints when they receive notification from a client that it has disconnected its channel. Supports invalidating the HttpFlexSession and underlying JEE HttpSession
    ...
    public void invalidate(boolean recreate)
    {
        ...
        if (recreate)
        {
            ...
            if (req != null)
            {
                ...
                FlexContext.setThreadLocalObjects(FlexContext.getFlexClient(), getFlexSession(req), FlexContext.getMessageBroker(), req, FlexContext.getHttpResponse(), FlexContext.getServletConfig());
            }
    ...
    /**
     * Implements HttpSessionListener. HttpSession created events are handled by setting an internal flag indicating that registration as an HttpSession listener was successful and we will be notified
     * of session attribute changes and session destruction. NOTE: This method is not invoked against an HttpFlexSession associated with a request handling thread.
     */
    @Override
    ...
    /**
     * Implements HttpSessionListener. When an HttpSession is destroyed, the associated HttpFlexSession is also destroyed. NOTE: This method is not invoked against an HttpFlexSession associated with a
    ...
    /**
     * Implements HttpSessionBindingListener. This callback will destroy the HttpFlexSession upon being unbound, only in the case where we haven't been registered as an HttpSessionListener in web.xml
     * and can't shut down based on the HttpSession being invalidated. NOTE: This method is not invoked against an HttpFlexSession associated with a request handling thread.
     */
    @Override
    ...
    /**
     * @exclude Invoked by HttpSessionListener or binding listener on HttpSession invalidation to invalidate the wrapping FlexSession.
     */
    ...
    /**
     * Map of HttpSession Ids to FlexSessions. We need this when registered as a listener in web.xml in order to trigger the destruction of a FlexSession when its associated HttpSession is
     * invalidated/destroyed. The Servlet spec prior to version 2.4 defined the session destruction event to be dispatched after attributes are unbound from the session so when we receive notification
     * that an HttpSession is destroyed there's no way to get to the associated FlexSession attribute because it has already been unbound... Additionally, we need it to handle attribute removal events
     * that happen during HttpSession destruction because the FlexSession can be unbound from the session before the other attributes we receive notification for.
     * 
     * Because of this, it's simplest to just maintain this lookup table and use it for all HttpSession related event handling.
     * 
     * The table is maintained on the servlet context instead of statically in order to prevent collisions across web-apps.
     */
    private Map getHttpSessionToFlexSessionMap(HttpSession session)
    {
        try
        {
            ...
            if (map == null)
            {
                ...
                if (Log.isError())
                    Log.getLogger(FLEX_SESSION_LOG_CATEGORY).error("HttpSession to FlexSession map not created in message broker for " + session.getId());
        ...
        {
            if (Log.isDebug())
                Log.getLogger(FLEX_SESSION_LOG_CATEGORY).debug("Unable to get HttpSession to FlexSession map for " + session.getId() + " " + e.toString());
