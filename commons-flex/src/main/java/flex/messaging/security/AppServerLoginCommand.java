...
/**
 * This class implements LoginCommand and doAuthorization in way that should work by default if authorization logged a user into the J2EE application server. doAuthorization uses isUserInRole.
 * 
 * @exclude
 */
public abstract class AppServerLoginCommand implements LoginCommand
{
     ...
     * 
     * @param principal
     *            The principal being checked for authorization
     * @param roles
     *            A List of role names to check, all members should be strings
    ...
    /*
     * TODO UCdetector: Remove unused code: protected String extractPassword(Object credentials) { String password = null; if (credentials instanceof String) { password = (String)credentials; } else
     * if (credentials instanceof Map) { password = (String)((Map)credentials).get(MessageIOConstants.SECURITY_CREDENTIALS); } return password; }
     */
