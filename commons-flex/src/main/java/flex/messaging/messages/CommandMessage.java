...
public class CommandMessage extends AsyncMessage
{
    ...
    /**
     * This operation is used to send credentials to the endpoint so that the user can be logged in over the current channel. The credentials need to be Base64 encoded and stored in the
     * <code>body</code> of the message.
     */
    ...
    /**
     * Header to suppress poll response processing. If a client has a long-poll parked on the server and issues another poll, the response to this subsequent poll should be tagged with this header in
     * which case the response is treated as a no-op and the next poll will not be scheduled. Without this, a subsequent poll will put the channel and endpoint into a busy polling cycle.
     */
    public static final String NO_OP_POLL_HEADER = "DSNoOpPoll";

    /**
     * @exclude Internal header used to tag poll messages when a poll-wait must be suppressed.
     */
    ...
    /**
     * @exclude The position of the operation flag within all flags. Constant used during serialization.
     */
    ...
    /**
     * The operation names that map to each of the operation constants above. The constants in this list should remain parallel to the above constants
     */
    static final String[] operationNames = { "subscribe", "unsubscribe", "poll", "unused3", "client_sync", "client_ping", "unused6", "cluster_request", "login", "logout", "subscription_invalidate", "multi_subscribe", "disconnect", "trigger_connect" };
    ...
    /**
     * Constructs a <code>CommandMessage</code> instance. The message id is set to a universally unique value, and the timestamp for the message is set to the current system timestamp. The operation
     * is set to a default value of <code>UNKNOWN_OPERATION</code>.
     */
    ...
    /**
     * Constructs a <code>CommandMessage</code> instance. The message id is set to a universally unique value, and the timestamp for the message is set to the current system timestamp.
     * 
     * @param operation
     *            The operation for the <code>CommandMessage</code>; one of the operation constants.
     */
     ...
     * 
     * @param operation
     *            The operation for this <code>CommandMessage</code>.
     */
    ...
    /**
     * @exclude Debugging function which returns the name of the operation for a given operation code.
     */
    ...
    /**
     * @exclude Utility method to pretty print a <code>CommandMessage</code>.
     * 
     * @param indentLevel
     *            This method may be invoked recursively so this argument allows nested messages to print relative to the current print stack.
     */
    @Override
    ...
    /**
     * @exclude Utility method to build the log category to use for logging <code>CommandMessage</code>s.
     */
    @Override
