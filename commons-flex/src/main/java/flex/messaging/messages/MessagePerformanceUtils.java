 ...
 * @exclude
 * 
 *          Utility class for populating MessagePerformanceInformation objects at various stages of server processing. A given message may have three MPI headers populated (naming convention of these
 *          headers is from the perspective of the server):
 * 
 *          DSMPII - incoming message, message sent from client to server DSMPIO - outgoing message, response/acknowledgement message sent from server back to client DSMPIP - only populated for a
 *          pushed message, this is information for the incoming message that caused the pushed message
 */
public class MessagePerformanceUtils
{
     ...
     * @exclude
     * 
     *          Clones the MPI object for the incoming message from client to server from the batch wrapper to all messages included in it, keeping track of overhead time spent doing this.
     * 
     * @param message
     *            The message whose MPI should be propogated
     */
     ...
     * @exclude
     * 
     *          This method finalizes the incoming MPI instance. It is necessary because the client send time is stored on the incoming MPII and the server receive time as well as message size are
     *          populated in the MPI object stored on the ActionContext, this method combines the information into one MPI instance.
     * 
     * @param context
     *            - The action context used to deserialize the incoming message, it will have the server receive time and message size information
     * @param inMessage
     *            - The incoming message, its MPI will have the client send time
     */
    public static void setupMPII(ActionContext context, Message inMessage)
    {
        ...
        {
            if (Log.isDebug())
                Log.getLogger(LOG_CATEGORY).error("MPI error: setting up response MPI : " + e.toString());
        }
    }
     ...
     * @exclude
     * 
     *          This method sets up the outgoing MPI object for a response/acknowledgement message. The outgoing message should also have a copy of the incoming MPI so that when the client receives
     *          the response it has access to all information
     * 
     * @param context
     *            - The context used to deserialize the incoming message
     * @param inMessage
    ...
    public static void updateOutgoingMPI(ActionContext context, Message inMessage, Object outMessage)
    {
        try
        {
            ...
            if (outMessage instanceof CommandMessage && ((CommandMessage) outMessage).getOperation() == CommandMessage.CLIENT_SYNC_OPERATION)
            {
                // pushed message case
                    ...
                    if (origMPII == null || MessagePerformanceUtils.getMPII(inMessage) == null)
                    {
                        ...
                        if (Log.isError())
                        {
                            Log.getLogger(LOG_CATEGORY).error(
                                            "MPI is enabled but could not get message performance information " + "for incoming MPI instance from client message.  The client " + "might have created a new channel not configured to send message"
                                                            + " performance after declaring a different destination which does." + "  The client channel should either be configured to add MPI " + "properties, or a server destion which has the same MPI "
     ...
     * 
     * @param message
     *            The message whose MPI header will be set
     * @param mpi
     *            The incoming MPI instance
     */
     ...
     *            The message whose MPI header will be set
     * @param mpi
     *            The pushed MPI instance (this is the incoming MPI instance of the message that caused the push)
     */
     ...
     * 
     * @param message
     *            The message whose MPI header will be retrieved
     ...
     * @param message
     *            The message whose MPI header will be retrieved
     * @return mpi Pushed MPI instance (this is the incoming MPI instance of the message that caused the push)
     */
     ...
     * @exclude
     * 
     *          Convenience method for setting server pre-push processing time on a message. No-op if record-message-times is false
     * 
     * @param message
     *            The message whose MPI header will be updated
     */
     ...
     * @exclude
     * 
     *          Convenience method for setting server pre-adapter timestamp on a message. No-op if record-message-times is false
     * 
     * @param message
     *            The message whose MPI header will be updated
     */
     ...
     * @exclude
     * 
     *          Convenience method for setting server post-adapter timestamp on a message. No-op if record-message-times is false
     * 
     * @param message
     *            The message whose MPI header will be updated
     */
    ...
    /**
     * 
     * Method may be called from a custom adapter to mark the beginning of processing that occurs outside of the adapter for a particular message. Use this method in conjunction with
     * <code>markServerPostAdapterExternalTime</code> to mark the amount of time spent when your adapter must make a call to an external component. If <code>record-message-times</code> is
     * <code>true</code> for the communication channel, the server processing time external to the adapter may be retrieved via MessagePerformanceUtils.serverAdapterExternalTime on the client once it
     * receives the message.
     * 
     * If <code>record-message-times</code> is <code>false</code> for the communication channel, calling this method will have no effect.
     * 
     * @param message
    ...
    /**
     * 
     * Method may be called from a custom adapter to mark the end of processing that occurs outside of the adapter for a particular message. Use this method in conjunction with
     * <code>markServerPreAdapterExternalTime</code> to mark the amount of time spent when your adapter must make a call to an external component. If <code>record-message-times</code> is
     * <code>true</code> for the communication channel, the server processing time external to the adapter may be retrieved via MessagePerformanceUtils.serverAdapterExternalTime on the client once it
     * receives the message.
     * 
     * If <code>record-message-times</code> is <code>false</code> for the communication channel, calling this method will have no effect.
