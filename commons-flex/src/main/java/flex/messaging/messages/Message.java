...
/**
 * Messages are sent from Endpoints into the MessageBroker, which then sends them to a Service. The MessageBroker also sends Messages to Endpoints, and a Service may ask its Broker to send Messages to
 * Endpoints.
 * 
 * @author neville
 */
public interface Message extends Serializable, Cloneable // , javax.jms.Message
{
    ...
    /**
     * @exclude Used internally to enable/disable validation of the channel endpoint that a message for a destination arrived over.
     */
     ...
     * 
     * @param value
     *            The client id to set for the message.
     */
     ...
     * 
     * @param value
     *            The headers to set on the message.
     */
    void setHeaders(Map value);

    /**
     * Returns a header value corresponding to the passed header name. If no header with this name exists, <code>null</code> is returned.
     * 
     * @param name
     *            The header name to retrieve a value for.
     ...
     * 
     * @param name
     *            The header to test for existence.
