...
/**
 * The channel configuration is intentionally generic so that other channels can be added in the future. This format also allows server-endpoint specific and client code-generation specific settings
 * to be modified without affecting needing to update the configuration parser.
 * 
...
public class ChannelSettings extends PropertiesSettings
{
    ...
    /*
     * TODO UCdetector: Remove unused code: public ChannelSettings(String id) { this.id = id; }
     */
     ...
     * 
     * @return the port number for this channel or 0 if channel url does not contain a port number
     */
    public int getPort()
    {
        return port;
    ...
    /**
     * In this client version of the URI parser we're just looking to replace the context root tokens.
     */
    ...
    /**
     * Remove protocol, host, port and context-root from a given url. Unlike parseClientUri, this method does not check if the channel setting has been parsed before.
     * 
     * @param url
     *            Original url.
