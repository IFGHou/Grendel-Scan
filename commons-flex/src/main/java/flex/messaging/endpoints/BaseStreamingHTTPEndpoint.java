...
/**
 * Base for HTTP-based endpoints that support streaming HTTP connections to connected clients. Each streaming connection managed by this endpoint consumes one of the request handler threads provided
 * by the servlet container, so it is not highly scalable but offers performance advantages over client polling for clients receiving a steady, rapid stream of pushed messages. This endpoint does not
 * support polling clients and will fault any poll requests that are received. To support polling clients use subclasses of BaseHTTPEndpoint instead.
 */
public abstract class BaseStreamingHTTPEndpoint extends BaseHTTPEndpoint
{
    ...
    /**
     * This is the token at the end of the HTTP request line that indicates that it's a stream connection that we should hold open to push data back to the client over as opposed to a regular
     * request-response message.
     */
    private static final String OPEN_COMMAND = "open";

    /**
     * This is the token at the end of the HTTP request line that indicates that it's a stream connection that we should close.
     */
     ...
     *            <code>true</code> if the <code>BaseStreamingHTTPEndpoint</code> is manageable; otherwise <code>false</code>.
     */
    ...
    /**
     * Used to keep track of the mapping between user agent match strings and the bytes needed to kickstart their streaming connections.
     */
    protected UserAgentManager userAgentManager = new UserAgentManager();

    /**
     * This flag is volatile to allow for consistent reads across thread without needing to pay the cost for a synchronized lock for each read.
     */
    ...
    /**
     * A Map(EndpointPushNotifier, Boolean.TRUE) containing all currently open streaming notifiers for this endpoint. Used for clean shutdown.
     */
    ...
    /**
     * Returns the number of milliseconds the server will wait before writing a single null byte to the streaming connection to make sure the client is still available when there are no new messages
     * for the client. A non-positive value means server will wait forever for new messages and it will not write the null byte to determine if the client is available.
     */
     ...
     * 
     * @return The number of minutes a client can remain idle before the server times the connection out.
     */
     ...
     * 
     * @return The maximum number of clients that will be allowed to establish a streaming HTTP connection with the endpoint.
     */
     ...
     * 
     * @param maxStreamingClients
     *            The maximum number of clients that will be allowed to establish a streaming HTTP connection with the server.
     */
    ...
    /**
     * @exclude Returns a <code>ConfigMap</code> of endpoint properties that the client needs. This includes properties from <code>super.describeEndpoint</code> and additional
     *          <code>BaseHTTPEndpoint</code> specific properties under "properties" key.
     */
    @Override
    ...
    /**
     * Handles HTTP requests targetting this endpoint. Two types or requests are supported. If the request is a regular request-response AMF/AMFX message it is handled by the base logic in
     * BaseHTTPEndpoint.service. However, if it is a request to open a streaming HTTP connection to the client this endpoint performs some validation checks and then holds the connection open to
     * stream data back to the client over.
     * 
     * @param req
     *            The original servlet request
     * @param res
     *            The active servlet response
     */
    @Override
    public void service(HttpServletRequest req, HttpServletResponse res)
    {
        ...
        if (command != null)
            ...
            // Let BaseHTTPEndpoint logic handle regular request-response messaging.
    ...
    /**
     * If the message has MPI enabled, this method adds all the needed performance headers to the message.
     * 
     * @param message
     *            Message to add performance headers to.
     */
    ...
    /**
     * Used internally for performance information gathering; not intended for public use. The default implementation of this method returns zero. Subclasses should overwrite if they want to
     * accurately report message size information in performance information gathering.
     * 
     * @param message
     *            Message to get the size for.
     * 
     ...
     * 
     * @param req
     *            The <code>HttpServletRequest</code> to service.
     * @param res
     *            The <code>HttpServletResponse</code> to be used in case an error has to be sent back.
     * @param flexClient
     *            FlexClient that requested the streaming connection.
     */
    protected void handleFlexClientStreamingOpenRequest(HttpServletRequest req, HttpServletResponse res, FlexClient flexClient)
    {
        ...
        if (canStream && session.canStream)
        {
            ...
            {
                if (Log.isError())
                    log.error("Endpoint with id '" + getId() + "' cannot grant streaming connection to FlexClient with id '" + flexClient.getId() + "' because " + MAX_STREAMING_CLIENTS + " limit of '" + maxStreamingClients + "' has been reached.");
                try
                {
            ...
            {
                if (Log.isInfo())
                    log.info("Endpoint with id '" + getId() + "' cannot grant streaming connection to FlexClient with id '" + flexClient.getId() + "' because " + UserAgentManager.MAX_STREAMING_CONNECTIONS_PER_SESSION + " limit of '"
                                    + session.maxConnectionsPerSession + ((agentSettings != null) ? "' for user-agent '" + agentSettings.getMatchOn() + "'" : "") + " has been reached.");
                try
                {
                    // Return an HTTP status code 400.
                    res.sendError(HttpServletResponse.SC_BAD_REQUEST);
            ...
            try
            {
                ...
                {
                    if (Log.isDebug())
                        log.debug("Endpoint with id '" + getId() + "' is streaming " + kickStartBytesToStream.length + " bytes (not counting chunk encoding overhead) to kick-start the streaming connection for FlexClient with id '" + flexClient.getId() + "'.");
                ...
                catch (MessageException me)
                {
                    ...
                    {
                        if (Log.isWarn())
                            log.warn("Endpoint with id '" + getId() + "' received a duplicate streaming connection request from, FlexClient with id '" + flexClient.getId() + "'. Faulting request.");
                    ...
                    synchronized (notifier.pushNeeded)
                    {
                        try
                        {
                            ...
                            if (messages == null && serverToClientHeartbeatMillis > 0)
                            {
                                ...
                                {
                                    if (Log.isWarn())
                                        log.warn("Endpoint with id '" + getId() + "' is closing the streaming connection to FlexClient with id '" + flexClient.getId() + "' because endpoint encountered a socket write error"
        ...
        else
        {
            if (Log.isError())
            {
                ...
                if (!canStream)
                {
                    logString = "Endpoint with id '" + getId() + "' cannot grant streaming connection to FlexClient with id '" + flexClient.getId() + "' because " + MAX_STREAMING_CLIENTS + " limit of '" + maxStreamingClients + "' has been reached.";
                }
                else if (!session.canStream)
                {
                    logString = "Endpoint with id '" + getId() + "' cannot grant streaming connection to FlexClient with id '" + flexClient.getId() + "' because " + UserAgentManager.MAX_STREAMING_CONNECTIONS_PER_SESSION + " limit of '"
     ...
     *            FlexClient that requested the streaming connection.
     * @param streamId
     *            The id for the stream to close.
     */
    ...
    protected void serviceStreamingRequest(HttpServletRequest req, HttpServletResponse res)
    {
        ...
        {
            if (Log.isError())
                log.error("Endpoint with id '" + getId() + "' cannot service the streaming request made with " + " HTTP 1.0. Only HTTP 1.1 is supported.");
        ...
        {
            if (Log.isError())
                log.error("Endpoint with id '" + getId() + "' cannot service the streaming request as the supplied command '" + command + "' is invalid.");
        ...
        {
            if (Log.isError())
                log.error("Endpoint with id '" + getId() + "' cannot service the streaming request as no FlexClient id" + " has been supplied in the request.");
        ...
        {
            if (Log.isError())
                log.error("Endpoint with id '" + getId() + "' cannot service the streaming request as either the supplied" + " FlexClient id '" + flexClientId + " is not valid, or the FlexClient with that id is not valid.");
    ...
    /**
     * Helper method to write a chunk of bytes to the output stream in an HTTP "Transfer-Encoding: chunked" format. If the bytes array is null or empty, a terminal chunk will be written to signal the
     * end of the response. Once the chunk is written to the output stream, the stream will be flushed immediately (no buffering).
     * 
     * @param bytes
     *            The array of bytes to write as a chunk in the response; or if null, the signal to write the final chunk to complete the response.
     * @param os
     *            The output stream the chunk will be written to.
     * @param response
     *            The HttpServletResponse, used to flush the chunk to the client.
     * 
     * @throws IOException
     *             if writing the chunk to the output stream fails.
     */
    ...
    /**
     * Helper method invoked by the endpoint request handler thread cycling in wait-notify. Serializes messages and streams each to the client as a response chunk using streamChunk().
     * 
     * @param messages
     *            The messages to serialize and push to the client.
     * @param os
     *            The output stream the chunk will be written to.
     * @param response
     *            The HttpServletResponse, used to flush the chunk to the client.
     ...
     * 
     * @param notifier
     *            The EndpointPushNotifier to monitor.
     */
