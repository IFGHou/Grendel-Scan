...
/**
 * Extension to the HTTPEndpoint to support streaming HTTP connections to connected clients. Each streaming connection managed by this endpoint consumes one of the request handler threads provided by
 * the servlet container, so it is not highly scalable but offers performance advantages over client polling for clients receiving a steady, rapid stream of pushed messages. This endpoint does not
 * support polling clients and will fault any poll requests that are received. To support polling clients use HTTPEndpoint instead.
 */
public class StreamingHTTPEndpoint extends BaseStreamingHTTPEndpoint
{
     ...
     *            <code>true</code> if the <code>StreamingHTTPEndpoint</code> is manageable; otherwise <code>false</code>.
     */
    ...
    /**
     * Used internally for performance information gathering; not intended for public use. Serializes the message in AMFX format and returns the size of the serialized message.
     * 
     * @param message
    ...
    /**
     * Invoked automatically to allow the <code>StreamingHTTPEndpoint</code> to setup its corresponding MBean control.
     * 
     * @param broker
     *            The <code>MessageBroker</code> that manages this <code>StreamingHTTPEndpoint</code>.
     */
    @Override
    ...
    /**
     * Determines and returns the correct AmfxOutput implementation for the JVM is use.
     * 
