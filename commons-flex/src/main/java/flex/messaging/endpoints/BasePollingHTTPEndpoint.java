...
/**
 * Base for HTTP-based endpoints that support regular polling and long polling which means placing request threads that are polling for messages into a wait state until messages are available to
 * deliver or the configurable wait interval is reached.
 */
public abstract class BasePollingHTTPEndpoint extends BaseHTTPEndpoint implements PollWaitListener
{
     ...
     *            <code>true</code> if the <code>BasePollingHTTPEndpoint</code> is manageable; otherwise <code>false</code>.
     */
    ...
    /**
     * Set when properties are handled; used as a shortcut for logging to determine whether this instance attempts to put request threads in a wait state or not.
     */
    ...
    /**
     * Returns the number of milliseconds the client will wait after receiving a response for a poll with server wait before it issues its next poll request. A value of zero or less causes the client
     * to use its default polling interval (based on the channel's polling-interval-millis configuration) and this value is ignored. A value greater than zero will cause the client to wait for the
     * specified interval before issuing its next poll request with a value of 1 triggering an immediate poll from the client as soon as a waited poll response is received.
     */
    ...
    /**
     * Sets the number of milliseconds a client will wait after receiving a response for a poll with server wait before it issues its next poll request. A value of zero or less causes the client to
     * use its default polling interval (based on the channel's polling-interval-millis configuration) and this value is ignored. A value greater than zero will cause the client to wait for the
     * specified interval before issuing its next poll request with a value of 1 triggering an immediate poll from the client as soon as a waited poll response is received. This property does not
     * effect polling clients that poll the server without a server wait.
     * 
     * @value The number of milliseconds a client will wait before issuing its next poll when the server is configured to wait.
     */
     ...
     *            The maximum number of server poll response threads that will be waiting for messages to arrive for the client.
     */
     ...
     * 
     * @param waitInterval
     *            The number of milliseconds the server poll response thread will be waiting for messages to arrive for the client.
     */
    ...
    /**
     * Overrides the base poll handling to support optionally putting Http request handling threads into a wait state until messages are available to be delivered in the poll response or a timeout is
     * reached. The number of threads that may be put in a wait state is bounded by <code>max-waiting-poll-requests</code> and waits will only be attempted if the canWait flag that is based on the
     * <code>max-waiting-poll-requests</code> and the specified <code>wait-interval</code> is true.
     * 
     * @param flexClient
