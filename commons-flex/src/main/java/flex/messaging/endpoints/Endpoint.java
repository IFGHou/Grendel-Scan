...
/**
 * An endpoint receives messages from clients and decodes them, then sends them on to a MessageBroker for routing to a service. The endpoint also encodes messages and delivers them to clients.
 * Endpoints are specific to a message format and network transport, and are defined by the named URI path on which they are located.
 * 
 * @author neville
 */
public interface Endpoint extends Manageable
{
    ...
    /**
     * Returns a <code>ConfigMap</code> of endpoint properties that the client needs.
     */
    ...
    /**
     * All endpoints must be managed by a single MessageBroker, and must be capable of returning a reference to that broker. This broker reference is used when the endpoint wishes to send a message to
     * one of the broker's services.
     * 
