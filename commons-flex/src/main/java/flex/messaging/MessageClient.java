...
/**
 * Represents a client-side MessageAgent instance. Currently a server-side MessageClient is only created if its client-side counterpart has subscribed to a destination for pushed data (e.g. Consumer).
 * Client-side Producers do not result in the creation of corresponding server-side MessageClient instances.
 * 
 * Client-side MessageAgents communicate with the server over a Channel that corresponds to a FlexSession. Server-side MessageClient instances are always created in the context of a FlexSession and
 * when the FlexSession is invalidated any associated MessageClients are invalidated as well.
 * 
 * MessageClients may also be timed out on a per-destination basis and this is based on subscription inactivity. If no messages are pushed to the MessageClient within the destination's subscription
 * timeout period the MessageClient will be shutdown even if the associated FlexSession is still active and connected. Per-destination subscription timeout is an optional configuration setting, and
 * should only be used when inactive subscriptions should be shut down opportunistically to preserve server resources.
 */
public class MessageClient extends TimeoutAbstractObject implements Serializable
{
    ...
    /**
     * Serializable to support broadcasting subscription state across the cluster for optimized message routing.
     */
    ...
    /**
     * @exclude Constructs a new MessageClient for local use.
     * 
     * @param clientId
     ...
     * @param endpointId
     *            The Id of the endpoint this MessageClient subscription was created over.
     */
    ...
    /**
     * @exclude Constructs a new MessageClient.
     * 
     * @param clientId
     *            The clientId for the MessageClient.
     * @param destination
     ...
     *            The Id of the endpoint this MessageClient subscription was created over.
     * @param useSession
     *            RemoteMessageClient instances should not be associated with a FlexSession (pass false).
     */
    ...
    /**
     * This flag is set to true when the client channel that this subscription was established over is disconnected. It supports cleaning up per-endpoint outbound queues maintained by the FlexClient.
     * If the client notifies the server that its channel is disconnecting, the FlexClient does not need to maintain an outbound queue containing a subscription invalidation message for this
     * MessageClient to send to the client.
     */
    ...
    /**
     * The FlexSession associated with the MessageClient. Not final because this needs to be reset if the subscription fails over to a new endpoint.
     */
    ...
    /**
     * A counter used to control invalidation for a MessageClient that has multiple subscriptions to its destination. Unsubscribing from one will not invalidate the MessageClient as long as other
    ...
    /**
     * Has anyone explicitly registered this message client. This indicates that there is a reference to this MessageClient which is not an explicit subscription. This is a hook for FDMS and other
     * adapters which want to use pushMessageToClients with clientIds but that do not want the subscription manager to manage subscriptions for them.
     */
    ...
    /**
     * @exclude This is used for FlexClient outbound queue management. When a MessageClient is invalidated if it is attempting to notify the client, then we must leave the outbound queue containing
     *          the notification in place. Otherwise, any messages queued for the subscription may be removed from the queue and possibly shut down immediately.
     */
    ...
    /**
     * @exclude This is set to true when the MessageClient is invalidated due to the client channel the subscription was established over disconnecting. It allows the FlexClient class to cleanup the
     *          outbound queue for the channel's corresponding server endpoint for the remote client, because we know that no currently queued messages need to be retained for delivery.
     */
    ...
    /**
     * @exclude This is true when some code other than the SubscriptionManager is maintaining subscriptions for this message client. It ensures that we have this MessageClient kept around until the
     *          session expires.
     */
    ...
    /**
     * @exclude Adds a subscription to the subscription set for this MessageClient.
     * 
     * @param selector
     *            The selector expression used for the subscription.
     * @param subtopic
     *            The subtopic used for the subscription.
     */
    ...
    /**
     * @exclude Removes a subscription from the subscription set for this MessageClient.
     * 
     * @param selector
     *            The selector expression for the subscription.
     * @param subtopic
     *            The subtopic for the subscription.
    ...
    /**
     * @exclude We use the same MessageClient for more than one subscription with different selection criteria. This tracks the number of subscriptions that are active so that we know when we are
     *          finished.
     */
    ...
    /**
     * @exclude Decrements the numReferences variable and returns true if this was the last reference.
     */
    ...
    /**
     * @exclude Invoked by SubscriptionManager once the subscription state is setup completely for the MessageClient..
     */
    ...
    /**
     * @exclude Invoked by SubscriptionManager while handling a subscribe request. If the request is updating an existing subscription the 'push' state in the associated FlexClient may need to be
     *          updated to ensure that the correct endpoint is used for this subscription.
     * 
     * @param newEndpointId
     *            The id for the new endpoint that the subscription may have failed over to.
     */
    ...
    /**
     * @exclude Used to test whether this client should receive this message based on the list of subscriptions we have recorded for it. It must match both the subtopic and the selector expression.
     *          Usually this is done by the subscription manager - this logic is only here to maintain api compatibility with one of the variants of the pushMessageToClients which has subscriberIds
     *          and an evalSelector property.
     * 
     * @param message
     *            The message to test.
     */
        ...
        synchronized (lock)
        {
            for (Iterator it = subscriptions.iterator(); it.hasNext();)
            {
                ...
                catch (JMSSelectorException jmse)
                {
                    ...
                    if (Log.isWarn())
                    {
                        Log.getLogger(JMSSelector.LOG_CATEGORY).warn("Error processing message selector: " + jmse.toString() + StringUtils.NEWLINE + "  incomingMessage: " + message + StringUtils.NEWLINE + "  selector: " + csel + StringUtils.NEWLINE);
                    }
                    return false;
                }
            }
    ...
    /**
     * Invalidates the MessageClient, and optionally attempts to notify the client that this subscription has been invalidated. This overload is used when a subscription is timed out while the client
     * is still actively connected to the server but should also be used by any custom code on the server that invalidates MessageClients but wishes to notify the client cleanly.
     * 
     * @param notifyClient
     *            <code>true</code> to notify the client that its subscription has been invalidated.
     */
     ...
     * 
     * @param message
     *            The message to push to the client before invalidating. When message is null, MessageClient is invalidated silently.
     */
    ...
    /**
     * @exclude Compares this MessageClient to the specified object. The result is true if the argument is not null and is a MessageClient instance with a matching clientId value.
     * 
     * @param o
     *            The object to compare this MessageClient to.
    ...
    /**
     * @exclude Returns the hash code for this MessageClient. The returned value is the hash code for the MessageClient's clientId property.
     * 
    ...
    /**
     * @exclude The String representation of this MessageClient is returned (its clientId value).
     * 
    ...
    /**
     * @exclude Implements TimeoutCapable. This method returns the timeout value configured for the MessageClient's destination.
     */
    @Override
    ...
    /**
     * @exclude Implements TimeoutCapable. This method is invoked when the MessageClient has timed out and it invalidates the MessageClient.
     */
    @Override
    public void timeout()
    {
    ...
    /**
     * @exclude Returns true if a timeout task is running for this MessageClient.
     */
    ...
    /**
     * @exclude Records whether a timeout task is running for this MessageClient.
     */
    ...
    /**
     * Represents a MessageClient's subscription to a destination. It captures the optional selector expression and subtopic for the subscription.
     */
    protected static class SubscriptionInfo implements Comparable
    {
        ...
        public boolean equals(Object o)
        {
            if (o instanceof SubscriptionInfo)
            {
                ...
                return equalStrings(other.selector, selector) && equalStrings(other.subtopic, subtopic);
            }
            return false;
        }
        ...
        public int hashCode()
        {
            return (selector == null ? 0 : selector.hashCode()) + (subtopic == null ? 1 : subtopic.hashCode());
        }

        /**
         * Compares the two subscription infos (being careful to ensure we compare in a consistent way if the arguments are switched).
         */
        @Override
