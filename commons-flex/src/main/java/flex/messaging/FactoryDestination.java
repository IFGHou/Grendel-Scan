...
public abstract class FactoryDestination extends Destination
{
     ...
     *            <code>true</code> if the <code>FactoryDestination</code> is manageable; otherwise <code>false</code>.
     */
     ...
     * 
     * @param properties
     *            Properties for the <code>FactoryDestination</code>.
     */
    @Override
    ...
    /**
     * Returns the factory of the <code>FactoryDestination</code>. Before a valid <code>FlexFactory</code> can be returned, <code>MessageBroker</code> and hence <code>Service</code> of the
    ...
    /**
     * Sets the factory of the <code>FactoryDestination</code>. <code>MessageBroker</code> has to know the factory before it can be assigned to the destination.
     * 
     * @param id
    ...
    /**
     * Returns the <code>FactoryInstance</code>. <code>FactoryInstance</code> stores configuration state used for retrieving an instance from the factory. This needs to be called after calling
     * <code>setSource</code> and <code>setScope</code> methods.
     * 
     */
     ...
     * 
     * @param properties
     *            Properties to be used while creating the <code>FactoryInstance</code>.
     */
    ...
    /**
     * Sets the scope of the <code>FactoryDestination</code> that is used in <code>FactoryInstance</code> creation. Scope cannot be changed to and from application scope once
     * <code>FactoryInstance</code> is initialized.
     * 
     * @param scope
     */
    public void setScope(String scope)
    {
        if (factoryInstance != null)
        {
            if (FlexFactory.SCOPE_APPLICATION.equals(this.scope) && !FlexFactory.SCOPE_APPLICATION.equals(scope))
            {
                if (Log.isWarn())
                    Log.getLogger(getLogCategory()).warn("Current scope is " + FlexFactory.SCOPE_APPLICATION + " and it cannot be changed to " + scope + " once factory instance is initialized.");
                return;
            }
            else if (!FlexFactory.SCOPE_APPLICATION.equals(this.scope) && FlexFactory.SCOPE_APPLICATION.equals(scope))
            {
                if (Log.isWarn())
    ...
    /**
     * Sets the source of the <code>FactoryDestination</code> that is used in <code>FactoryInstance</code> creation. Source cannot be changed once <code>FactoryInstance</code> is initialized and the
    ...
    public void setSource(String source)
    {
        if (factoryInstance != null)
        {
            ...
            {
                if (Log.isWarn())
                    Log.getLogger(getLogCategory()).warn("Source of the destination cannot be changed once " + "factory instance is already initialized and it has " + FlexFactory.SCOPE_APPLICATION + " scope");
                return;
            }
    ...
    /**
     * This method first calls stop on its superclass <code>Destination</code> and then removes any assemblers from the ServletContext or Session that are ready for removal. If an assembler is only
     * used by a single destination (attribute-id==destination-id) then it is removed. If an assembler is shared across destinations, (attribute-id!=destination-id) then it is only removed if its
     * reference count (maintained in <code>MessageBroker</code>) is down to zero
     */
    @Override
    public void stop()
    {
