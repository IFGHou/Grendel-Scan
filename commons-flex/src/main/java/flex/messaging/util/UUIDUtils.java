...
/**
 * Static class that creates GUIDs according to the principles laid out in <a href="http://www.opengroup.org/dce/info/draft-leach-uuids-guids-01.txt">UUIDs and GUIDs</a>. Specifically, these GUIDs
 * have elements specific to the time and place of their creation; a unique TOD value (but the low-level IEEE 802/MAC address of the host system is not determined). The rest of the GUID is a sequence
 * of random hex digits from a cryptographically strong random number generator.
 * 
 * <p>
 * This class differs from the Leach specification in a few ways. It does not use a persistent store to track the clock sequence and does not coordinate ids given out by 2 processes on the same
 * machine. It does not follow the formal clock sequence guidelines but instead uses random numbers for all but the timestamp.
 * </p>
 */
public class UUIDUtils
{
    ...
    /**
     * The spec indicates that our time value should be based on 100 nano second increments but our time granularity is in milliseconds. The spec also says we can approximate the time by doing an
     * increment when we dole out new ids in the same millisecond. We can fit 10,000 100 nanos into a single millisecond.
     */
    private static final int MAX_IDS_PER_MILLI = 10000;

    /**
     * Any given time-of-day value can only be used once; remember the last used value so we don't reuse them.
     * <p>
    ...
    /**
     * 
     * Use the createUUID function when you need a unique string that you will use as a persistent identifier in a distributed environment. To a very high degree of certainty, this function returns a
     * unique value; no other invocation on the same or any other system should return the same value.
     * 
     * @return a Universally Unique Identifier (UUID) Proper Format: `XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX' where `X' stands for a hexadecimal digit (0-9 or A-F).
    ...
    /**
     * @param secure
     *            Boolean indicating whether to create a secure UUID.
     * @see #createUUID()
     */
     ...
     * 
     * @param ba
     *            byte[] 16 bytes in length representing a 128-bit UID.
     * 
     * @return String representation of the UID, or null if an invalid byte[] is provided.
     */
    ...
    /**
     * A utility method to check whether a String value represents a correctly formatted UID value. UID values are expected to be in the format generated by createUID(), implying that only capitalized
     * A-F characters in addition to 0-9 digits are supported.
     * 
     * @param uid
     ...
     * 
     * @param uid
     *            String representing a 128-bit UID.
     * 
     * @return byte[] 16 bytes in length representing the 128-bits of the UID or null if the uid could not be converted.
     */
    ...
    private static synchronized long uniqueTOD()
    {
        ...
        if (currentTOD == lastUsedTOD)
        {
            numIdsThisMilli++;
            /*
             * Fall back to the old technique of sleeping if we allocate too many ids in one time interval.
             */
