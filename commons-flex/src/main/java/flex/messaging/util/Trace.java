 ...
 * 
 * Code as follows: if (Trace.foo) Trace.trace("trace msg"...);
 * 
 * Enable as follows: java -Dtrace.foo -Dtrace.foo2 -Dtrace.foo3 or -Dtrace.all
 * 
 * Special flags: -Dtrace.flex -- enables all tracing -Dtrace.foo -- enables tracing on foo subsystem -Dtrace.timeStamp -- timeStamp all output lines -Dtrace.caller -- print the Class:method caller
 * -Dtrace.stackLines=10 -- print 10 stack lines -Dtrace.stackPrefix=java.lang -- print the stack up to java.lang
 * 
...
public class Trace
{
    ...
    /**
     * Write the string as a line to the trace stream. If the "stack" property is enabled, then the caller's stack call is also shown in the date.
     * 
     * @param str
     *            string to write to the trace stream
     */
