...
/**
 * A <code>MessageReceiver</code> that receives messages from JMS using synchronous <code>javax.jms.MessageConsumer.receive</code> call.
 * 
 * @exclude
 */
class SyncMessageReceiver implements MessageReceiver
{
     ...
     * 
     * @param jmsConsumer
     *            JMSConsumer associated with the SynMessageReceiver.
     */
    ...
    public void startReceive()
    {
        ...
        {
            if (Log.isDebug())
                Log.getLogger(JMSAdapter.LOG_CATEGORY).debug(Thread.currentThread() + " JMS consumer sync receive thread for JMS destination '" + jmsConsumer.destinationJndiName + "' is starting to poll the JMS server for new messages.");
    ...
    class MessageReceiveThreadFactory implements ThreadFactory
    {
        ...
        /**
         * Creates a new message receive thread. Synchronized to uniquely identify each receive thread at construct time safely.
         * 
         * @param worker
         *            The runnable to assign to the new thread.
         */
        @Override
    ...
    /**
     * Message receive threads that perform sync javax.jms.MessageConsumer.receive calls.
     */
