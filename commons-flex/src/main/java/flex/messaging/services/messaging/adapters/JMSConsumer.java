...
public abstract class JMSConsumer extends JMSProxy implements ExceptionListener
{
     ...
     *             The thrown naming exception.
     * @throws JMSException
     *             The thrown JMS exception.
     */
    @Override
    public void start() throws NamingException, JMSException
    {
        ...
        if (Log.isInfo())
            Log.getLogger(JMSAdapter.LOG_CATEGORY).info("JMS consumer for JMS destination '" + destinationJndiName + "' is starting.");
    }

    /**
     * Stops the <code>JMSConsumer</code> by stopping its associated receiver adapter and closing the underlying <code>MessageConsumer</code>. It then calls <code>JMSProxy.close</code> for session and
     * connection closure.
     */
    @Override
    public void stop()
    {
        if (Log.isInfo())
            Log.getLogger(JMSAdapter.LOG_CATEGORY).info("JMS consumer for JMS destination '" + destinationJndiName + "' is stopping.");
        ...
        {
            if (Log.isWarn())
                Log.getLogger(JMSAdapter.LOG_CATEGORY).warn("JMS consumer for JMS destination '" + destinationJndiName + "' received an error while closing its underlying MessageConsumer: " + e.getMessage());
        }
    ...
    /**
     * Stops the <code>JMSConsumer</code> and unsubscribes a durable subscription if one exists. By default this method delegates to <code>stop()</code> and doesn't remove a durable subscription.
     * 
     * @param unsubscribe
     *            Determines whether to unsubscribe a durable subscription if one exists, or not.
     */
    ...
    /**
     * Sets the message listener of the underlying MessageConsumer. This method is not meant to be directly called as it is used internally by MessageReceivers that need to perform async message
     * delivery. Any future custom MessageReceiver implementations can use this method to set themselves as MessageListeners to the underlying MessageConsumer.
     * 
     * @param listener
     *            Message listener to set on the underlying MessageConsumer.
    ...
    /**
     * Returns the <code>MessageReceiver</code> used by the consumer to retrieve JMS messages.
     * 
    ...
    /**
     * Sets the <code>MessageReceiver</code> used by the consumer to retrieve JMS messages. This property should not change after startup.
     * 
     * @param messageReceiver
     *            The <code>MessageReceiver</code> used.
     */
    ...
    /**
     * Returns the selector expression used when the underlying <code>javax.jms.MessageConsumer</code> is created.
     * 
    ...
    /**
     * Sets the selector expression used when the underlying <code>javax.jms.MessageConsumer</code> is created. This property should not change after startup.
     * 
     ...
     * 
     * @param jmsMessage
     *            The new JMS message to acknowledge and dispatch.
     */
    ...
    {
        if (Log.isInfo())
            Log.getLogger(JMSAdapter.LOG_CATEGORY).info(Thread.currentThread() + " JMS consumer for JMS destination '" + destinationJndiName + "' is waiting forever until a new message arrives.");
    ...
    {
        if (Log.isInfo())
            Log.getLogger(JMSAdapter.LOG_CATEGORY).info(Thread.currentThread() + " JMS consumer for JMS destination '" + destinationJndiName + "' is waiting " + timeout + " ms for new message to arrive");
     ...
     * 
     * @param message
     *            The JMS message to acknowledge.
     */
    protected void acknowledgeMessage(Message message)
    {
        if (getAcknowledgeMode() == Session.CLIENT_ACKNOWLEDGE)
        {
            ...
            {
                if (Log.isInfo())
                    Log.getLogger(JMSAdapter.LOG_CATEGORY).info("JMS consumer for JMS destination '" + destinationJndiName + "' received an error in message acknowledgement: " + e.getMessage());
            }
        }
    }

    /**
     * Initializes the message receiver used by the <code>JMSConsumer</code>. If the message receiver has been manually set, it validates the message receiver. Otherwise, it initalizes an async
     * message receiver if it can, and falls back to sync message delivery if it cannot.
     * 
     * This method should be called by subclasses once there is an underlying <code>javax.jms.MessageConsumer</code>.
     */
        ...
        if (messageReceiverManuallySet && messageReceiver != null)
        {
            if (messageReceiver instanceof AsyncMessageReceiver)
            {
                ...
                try
                {
                    ...
                    if (Log.isInfo())
                        Log.getLogger(JMSAdapter.LOG_CATEGORY).info("JMS consumer for JMS destination '" + destinationJndiName + "' is using async message receiver.");
                }
                catch (JMSException jmsEx)
                {
                    // JMS consumer for JMS destination ''{0}'' is configured to use async message receiver but the application server does not allow ''{1}'' call used in async message receiver.
                    // Please switch to sync message receiver.
            ...
            else if (messageReceiver instanceof SyncMessageReceiver)
            {
                ...
                if (Log.isInfo())
                {
                    Log.getLogger(JMSAdapter.LOG_CATEGORY).info(
                                    "JMS consumer for JMS destination '" + destinationJndiName + "' is using sync message receiver" + " with sync-receive-interval-millis: " + smr.getSyncReceiveIntervalMillis() + ", sync-receive-wait-millis: "
                                                    + smr.getSyncReceiveWaitMillis());
                }
            }
        }
