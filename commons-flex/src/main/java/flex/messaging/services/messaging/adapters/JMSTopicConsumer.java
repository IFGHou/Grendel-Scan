...
public class JMSTopicConsumer extends JMSConsumer
{
     ...
     * 
     * @param settings
     *            JMS settings to use for initialization.
     */
    @Override
    ...
    /**
     * Verifies that the <code>JMSTopicConsumer</code> is in valid state before it is started. For <code>JMSTopicConsumer</code> to be in valid state, it needs durableSubscriptionName if
     * durableConsumers is true.
     */
    @Override
    ...
    public void start() throws NamingException, JMSException
    {
        ...
        if (durableConsumers)
        {
            ...
            {
                if (Log.isDebug())
                    Log.getLogger(JMSAdapter.LOG_CATEGORY).debug("JMS consumer for JMS destination '" + destinationJndiName + "' is setting its underlying connection's client id to " + durableSubscriptionName + " for durable subscription.");
            ...
            catch (Exception e)
            {
                ...
                {
                    if (Log.isWarn())
                        Log.getLogger(JMSAdapter.LOG_CATEGORY).warn("The proxied durable JMS subscription with name, " + durableSubscriptionName + " could not set its client id " + "on the topic connection because it was interrupted: " + ie.toString());
                }
                catch (ExecutionException ee)
                {
                    // JMS topic consumer for JMS destination ''{0}'' is configured to use durable subscriptions but the application server does not permit javax.jms.Connection.setClientID method
                    // needed to support durable subscribers. Set durable property to false.
                    MessageException me = new MessageException();
    ...
    /**
     * Stops the <code>JMSTopicConsumer</code> and unsubscribes a durable subscription if one exists.
     * 
     * @param unsubscribe
    ...
    public void stop(boolean unsubscribe)
    {
        if (unsubscribe)
        {
            ...
            if (durableConsumers)
            {
                ...
                {
                    if (Log.isWarn())
                        Log.getLogger(JMSAdapter.LOG_CATEGORY).warn("The proxied durable JMS subscription with name, " + durableSubscriptionName + " failed to unsubscribe : " + e.toString());
                }
            }
        }
        super.stop();
    ...
    /**
     * Sets whether consumers are durable or not. This property is optional and defaults to false and it should not changed after startup.
     * 
     * @param durableConsumers
    ...
    /**
     * Sets the durable subscription name that is used as clientID on the underlying connection when durable subscriptions are used. This property should not changed after startup.
     * 
    ...
    /**
     * Helper thread class to circumvent a Web/EJB container from preventing us from calling setClientId() on our topic connection. By spinning this out into a short lived thread the container loses
