...
public abstract class JMSProxy
{
    ...
    /**
     * Creates a new <code>JMSProxy</code> default default acknowledge mode of <code>javax.jms.Session.AUTO_ACKNOWLEDGE</code>.
     */
    public JMSProxy()
    {
    ...
    /**
     * Verifies that the <code>JMSProxy</code> is in valid state before it is started. For <code>JMSProxy</code> to be in valid state, it needs to have a connection factory name and destination jndi
     * name assigned.
     */
    ...
    /**
     * Starts the <code>JMSProxy</code>. The default implementation verifies that <code>JMSProxy</code> is in a valid state to be started and then initializes JNDI context and connection factory for
     * JMS. Subclasses should call <code>super.start</code>.
     * 
    ...
    /**
     * Stops the <code>JMSProxy</code> by stopping its associated session and connection.
     */
    public void stop()
    {
        try
        ...
        {
            if (Log.isWarn())
                Log.getLogger(JMSAdapter.LOG_CATEGORY).warn("JMS proxy for JMS destination '" + destinationJndiName + "' received an error while closing" + " its underlying Session: " + e.getMessage());
        }
        ...
        {
            if (Log.isWarn())
                Log.getLogger(JMSAdapter.LOG_CATEGORY).warn("JMS proxy for JMS destination '" + destinationJndiName + "' received an error while closing" + " its underlying Connection: " + e.getMessage());
        }
        ...
        {
            if (Log.isWarn())
                Log.getLogger(JMSAdapter.LOG_CATEGORY).warn("JMS proxy for JMS destination '" + destinationJndiName + "' received an error while closing" + " its underlying JNDI context: " + e.getMessage());
        }
    }
    ...
    /**
     * Sets the acknowledge mode used by the <code>JMSProxy</code>. Valid values are javax.jms.Session.AUTO_ACKNOWLEDGE, javax.jms.Session.CLIENT_ACKNOWLEDGE, javax.jms.Session.DUPS_OK_ACNOWLEDGE.
     * This property is optional and defaults to javax.jms.Session.AUTO_ACKNOWLEDGE.
     * 
     * @param acknowledgeMode
     *            An int representing the acknowledge mode used.
     */
    public void setAcknowledgeMode(int acknowledgeMode)
    {
        if (acknowledgeMode == Session.AUTO_ACKNOWLEDGE || acknowledgeMode == Session.CLIENT_ACKNOWLEDGE || acknowledgeMode == Session.DUPS_OK_ACKNOWLEDGE)
    ...
    /**
     * Sets the <code>initial-context-environment</code> property. This property is optional. This property should be change after startup.
     * 
     * @param env
    ...
    /**
     * Initializes the JNDI context needed by JMS. This should be called before any other initialize methods.
     */
    ...
    public static class ConnectionCredentials
    {
        ...
        /**
         * Creates a <code>ConnectionCredentials</code> instance with the supplied username and password.
         * 
         * @param username
