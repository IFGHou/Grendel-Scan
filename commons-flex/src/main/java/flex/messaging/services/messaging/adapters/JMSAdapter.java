...
public class JMSAdapter extends MessagingAdapter implements JMSConfigConstants, JMSExceptionListener, JMSMessageListener, MessageClientListener
{
     ...
     *            <code>true</code> if the <code>JMSAdapter</code> has a corresponding MBean control for management; otherwise <code>false</code>.
     */
     ...
     * 
     * @param id
     *            The id of the <code>JMSAdapter</code>.
     * @param properties
     *            Properties for the <code>JMSAdapter</code>.
     */
    @Override
    ...
    /**
     * Verifies that the <code>JMSAdapter</code> is in valid state before it is started.
     */
    @Override
    ...
    /**
     * Casts the <code>Destination</code> into <code>MessageDestination</code> and calls super.setDestination.
     * 
     * @param destination
     *            The destination of the adapter.
     */
    @Override
     ...
     * 
     * @param jmsSettings
     *            <code>JMSSettings</code> of the <code>JMSAdapter</code>.
     */
     ...
     * 
     * @param evt
     *            The <code>JMSExceptionEvent</code>.
     */
    @Override
    ...
    public Object manage(CommandMessage commandMessage)
    {
        ...
        else if (commandMessage.getOperation() == CommandMessage.UNSUBSCRIBE_OPERATION)
        {
            ...
            if (commandMessage.getHeader(CommandMessage.SUBSCRIPTION_INVALIDATED_HEADER) != null && ((Boolean) commandMessage.getHeader(CommandMessage.SUBSCRIPTION_INVALIDATED_HEADER)).booleanValue() || preserveDurable)
                unsubscribe = false;
    ...
    /**
     * Implements MessageClientListener. When a MessageClient is created, JMSAdapter looks up its JMSConsumers and if there is a JMSConsumer that has the same clientId as MessageClient, it adds the
     * MessageClient to its list clients. This helps in keeping both sides of the bridge (MessageClient <--> JMSConsumer) notified when there's a failure on either side of the bridge.
     * 
     * @param messageClient
     *            The newly created MessageClient.
     */
    @Override
    ...
    /**
     * Implements MessageClientListener. When a MessageClient is destroyed (usually due to session timeout), its corresponding JMS consumer is removed. Note that this might have already happened if
     * the client first unsubscribes and in that case, this is a no-op.
     * 
     * @param messageClient
     ...
     * 
     * @param clientId
     *            The identifier for the consumer to remove.
     */
     ...
     * 
     * @param clientId
     *            The clientId associated with the JMSConsumer to remove.
     * @param unsubscribe
     *            Whether to unsubscribe the durable subscription or not.
     * @param invalidate
     *            Whether to invalidate the MessageClient or not.
     * @param invalidateMessage
     *            A message to push to the client before consumer is removed and its MessageClient is invalidated. If the message is null, MessageClient is invalidated silently.
     */
     ...
     * 
     * @param consumer
     *            The JMSConsumer instance to remove.
     * @param unsubscribe
     *            Whether to unsubscribe the durable subscription or not.
     * @param invalidate
     *            Whether to invalidate the MessageClient or not.
    ...
    /**
     * Invoked automatically to allow the <code>JMSAdapter</code> to setup its corresponding MBean control.
     * 
     * @param destination
     *            The <code>Destination</code> that manages this <code>JMSAdapter</code>.
     */
    @Override
    ...
    /**
     * Prefixes a clientId with DURABLE_SUBSCRIBER_NAME_PREFIX to build a subscription name to be used in JMSConsumers with durable connections.
     */
    ...
    /**
     * Convert from a <code>javax.jms.Message</code> type to a <code>flex.messaging.messages.AsyncMessage</code> type. Supported types are <code>javax.jms.TextMessage</code> and
    ...
    /**
     * Invalidates the MessageClient associated with the JMSConsumer with the supplied error message.
     * 
     * @param consumer
     *            The JMSConsumer whose MessageClient will be invalidated.
     * @param message
     *            The error message to push out before invalidating the MessageClient. If the message is null, MessageClient is invalidated silently.
     */
    private void invalidateMessageClient(JMSConsumer consumer, flex.messaging.messages.Message message)
    {
        ...
        if (clientId != null && messageClients.containsKey(clientId))
        {
            ...
            if (Log.isInfo())
                Log.getLogger(JMSAdapter.LOG_CATEGORY).info("The corresponding MessageClient for JMS consumer for JMS destination '" + consumer.getDestinationJndiName() + "' is being invalidated");
    ...
    private void jms(ConfigMap properties)
    {
        ...
        if (jms != null)
        {
            ...
            boolean durable = getDestination() instanceof MessageDestination ? ((MessageDestination) getDestination()).getServerSettings().isDurable() : false;
                    ...
                    for (Iterator iter = props.iterator(); iter.hasNext();)
                    {
                        ...
                        if (prop instanceof ConfigMap)
                        {
                            ...
                            if (name.startsWith("Context."))
                            {
                                ...
                                {
                                    // A <property> element for the <initial-context-environment> settings for the ''{0}'' destination specifies an invalid javax.naming.Context field for its <name>:
                                    // {1}
                                    MessageException messageEx = new MessageException();
                                ...
                                {
                                    // A <property> element for the <initial-context-environment> settings for the ''{0}'' destination specifies an inaccessible javax.naming.Context field for its
                                    // <name>: {1}
                                    MessageException messageEx = new MessageException();
