...
public abstract class JMSProducer extends JMSProxy
{
    ...
    /**
     * Create a new JMSProducer with default delivery mode of <code>javax.jms.Message.DEFAULT_DELIVERY_MODE</code> and default message priority of <code>javax.jms.Message.DEFAULT_PRIORITY</code>.
     */
    public JMSProducer()
    {
        super();
    ...
    /**
     * Verifies that the <code>JMSProducer</code> is in valid state before it is started. For <code>JMSProducer</code> to be in valid state, it needs to have a message type assigned.
     */
    @Override
    protected void validate()
    {
        ...
        if (messageType == null || (!(messageType.equals(JMSConfigConstants.TEXT_MESSAGE) || messageType.equals(JMSConfigConstants.OBJECT_MESSAGE))))
        {
    ...
    public void start() throws NamingException, JMSException
    {
        ...
        if (Log.isInfo())
            Log.getLogger(JMSAdapter.LOG_CATEGORY).info("JMS producer for JMS destination '" + destinationJndiName + "' is starting.");
    }

    /**
     * Stops the <code>JMSProducer</code> by closing its underlying <code>MessageProducer</code>. It then calls <code>JMSProxy.close</code> for session and connection closure.
     */
    @Override
    public void stop()
    {
        if (Log.isInfo())
            Log.getLogger(JMSAdapter.LOG_CATEGORY).info("JMS producer for JMS destination '" + destinationJndiName + "' is stopping.");
        ...
        {
            if (Log.isWarn())
                Log.getLogger(JMSAdapter.LOG_CATEGORY).warn("JMS producer for JMS destination '" + destinationJndiName + "' received an error while closing" + " its underlying MessageProducer: " + e.getMessage());
        }
    ...
    /**
     * Sets the delivery mode used by the <code>JMSProducer</code>. Valid values javax.jms.DeliveryMode.PERSISTENT and javax.jms.DeliveryMode.NON_PERSISTENT. This propery is optional and defaults to
    ...
    {
        if (deliveryMode == javax.jms.Message.DEFAULT_DELIVERY_MODE || deliveryMode == javax.jms.DeliveryMode.NON_PERSISTENT || deliveryMode == javax.jms.DeliveryMode.PERSISTENT)
    ...
    /**
     * Sets the message priority used by the <code>JMSProducer</code>. This property is optional and defaults to <code>javax.jms.Message.DEFAULT_PRIORITY</code>.
     * 
    ...
    /**
     * Sets the message type used by the <code>JMSProducer</code>. Supported types are <code>javax.jms.TextMessage</code> and <code>javax.jms.ObjectMessage</code>. This property is mandatory.
     * 
     * @param messageType
     *            String representing the message type used.
     */
