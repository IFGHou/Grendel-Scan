...
/**
 * The RemoteSubscriptionManager monitors subscriptions from other servers, not other clients. One MessageClient instance is used for each remote server. It clientId is the address of the remote
 * server. Using a separate instance of this class keeps the subscriptions of local clients separate from remote clients.
 * 
 * @author Jeff Vroom
 * @exclude
 */
public class RemoteSubscriptionManager extends SubscriptionManager implements RemoveNodeListener
{
    ...
    /*
     * A monitor lock used for synchronizing the attempt to request subscriptions across the cluster during startup.
     */
    ...
    public void addSubscriber(String flexClientId, Object clientId, String selector, String subtopic)
    {
        ...
        {
            /*
             * Only process subscriptions for servers whose subscription state we have received We may receive a subscribe/unsubscribe from a peer before we get their subscription state... we ignore
             * these since they will be included in the state we receive later
             */
    ...
    /**
     * Takes the selector and subtopic list from this address and for each one create a RemoteSubscription which gets registered in this table. We also register the remote subscription with a
     * "per server" index so we can easily remove them later on.
     */
    public void setSubscriptionState(Object state, Object address)
    {
        ...
        /*
         * need to be sure we do not accept any remote sub/unsub messages from a given server until we have received its subscription state
         * 
         * Also, we need to ensure we do not process any remote subscribe/unsubs from a remote server until we finish this list.
         */
        synchronized (syncLock)
        {
    ...
    /**
     * This method waits for some time for the receipt of the subscription state for the server with the given address. If we fail to receive a message after waiting for the 5 seconds, a warning is
     * printed.
     */
