...
/**
 * The <code>RemotingService</code> processes <code>RemotingMessage</code>s. A <code>RemotingMessage</code> informs a service adapter which method to invoke on a service, provides the input parameters
 * for the invocation, as well as any other settings relevant to the adapter, such as RunAs credential information for secured services.
 * <p>
 * The <code>RemotingService</code> must be initialized with configuration settings before it can successfully process any RemotingMessages.
 * </p>
 * <p>
 * Note that the <code>RemotingService</code> translates a destination into a service name or &quot;source&quot; that is meaningful to an adapter before invocation.
 * </p>
 * 
 * @author Sean Neville
...
public class RemotingService extends AbstractService
{
     ...
     *            <code>true</code> if the <code>RemotingService</code> is manageable; otherwise <code>false</code>.
     */
    ...
    /**
     * Creates a <code>RemotingDestination</code> instance, sets its id, sets it manageable if the <code>AbstractService</code> that created it is manageable, and sets its <code>Service</code> to the
     * <code>AbstractService</code> that created it.
     * 
     * @param id
     *            The id of the <code>RemotingDestination</code>.
    ...
    /**
     * Casts the <code>Destination</code> into <code>RemotingDestination</code> and calls super.addDestination.
     * 
     * @param destination
    ...
    /**
     * Processes messages of type <code>RemotingMessage</code> by invoking the requested destination&quot;s adapter.
     * <p>
     * Note that this method catches all Exceptions and throws only runtime <code>MessageException</code>s back to the MessageBroker as a best practice for the &quot;<code>MessageBroker</code> to
     * <code>Service</code>&quot; contract.
     * </p>
     * 
     * @param msg
     *            the <code>RemotingMessage</code> to process
    ...
    public Object serviceMessage(Message msg)
    {
        ...
        /*
         * if (isManaged()) { controller.incrementServiceMessageCount(); }
         */
    ...
    /**
     * Invoked automatically to allow the <code>RemotingService</code> to setup its corresponding MBean control.
     * 
     * @param broker
     *            The <code>MessageBroker</code> that manages this <code>RemotingService</code>.
     */
    @Override
