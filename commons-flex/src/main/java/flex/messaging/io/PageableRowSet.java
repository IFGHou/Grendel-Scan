...
/**
 * Implementations of this class are recoginized by the serialization filter as result sets that are pageable. A pageable result set is a server side cache of a query result (that implements
 * java.sql.ResultSet) and is typically stored in the session object. Users can request a subset of data from the object given that they know it's id.
 * 
...
public interface PageableRowSet
{
    ...
    /**
     * Use this method to get a subset of records. A map is returned with two fields, the first being the row number the data page started from, and the second being the array of arrays for the actual
     * data page.
     * 
    ...
    /**
     * If this function returns a number >= the total number of records in the recordset, then the recordset should be simply returned to the client in full. However, if it is < the total size, then
