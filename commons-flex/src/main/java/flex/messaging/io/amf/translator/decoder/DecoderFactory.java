...
/**
 * Utility class that tries to find an ActionScriptDecoder that will be able to convert the encoded object into an instance of the desired class.
 * 
...
public class DecoderFactory
{
    ...
    /**
     * A simple method for obtaining a placeholder or 'shell' object that will be subsequently populated by potentially some other deserializer. Decoders contain special logic to implement well known
     * Collections interfaces such as java.util.Map, java.util.Set, etc and also contain utilities to create native Arrays, so this functionality has been made available to other classes besides
     * decoders.
     * 
     * @param desiredClass
    ...
    /**
     * This is a faster implementation than getReferenceAwareDecoder as it doesn't track or care about restoring references in the event that an instance was converted to a new type. Since the all
     * MessageDeserializes now convert directly to strongly typed instances it is less likely that references will need to be tracked.
     * 
     * A case where a reference may have needed to be tracked would be that a Class has several properties of a subtype of Object[] (i.e. Array), Collection or Map but these properties were not of the
     * default type returned by the MessageDeserializer, that is ArrayList or HashMap, and that each property had the potential to point to the same instance. If we didn't track the reference, then
     * the conversion to the subtype (say HashMap -> TreeMap) would effectively clone these instances.
     * 
     * @param encodedObject
     * @param desiredClass
     * @return The <tt>ActionScriptDecoder</tt> to use for instances of the desired class.
    ...
    /**
     * A considerably slower entry point for decoders as it both changes the assumptions we can make about type translation and also keeps track of a lot of information when a complex type is
     * converted.
     * 
     * @param encodedObject
     * @param desiredClass
    ...
    /*
     * TODO UCdetector: Remove unused code: public static boolean isCharArray(Class desiredClass) { boolean isCharArray = false;
     * 
     * if (desiredClass.isArray()) { Class type = desiredClass.getComponentType(); if (type != null && type.equals(Character.TYPE)) { isCharArray = true; } }
     * 
     * return isCharArray; }
     */
