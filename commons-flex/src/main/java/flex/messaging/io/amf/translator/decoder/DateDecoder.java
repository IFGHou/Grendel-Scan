...
/**
 * Converts java.util.Date, java.sql.Date, java.util.Calendar or java.lang.Number (via longValue) instances to a java.util.Date, taking into consideration the SQL specific java.sql.Date type which is
 * required by Hibernate users.
 * 
 * If the incoming type was an AMF 3 Date, we remember the translation to Calendar in our list of known objects as Dates are considered complex objects and can be sent by reference. We want to retain
 * pointers to Date instances in our representation of an ActionScript object graph.
 * 
