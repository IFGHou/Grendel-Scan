...
/**
 * The <code>Destination</code> class is a source and sink for messages sent through a service destination and uses an adapter to process messages.
 */
public class Destination extends ManageableComponent implements java.io.Serializable
{
     ...
     *            <code>true</code> if the <code>Destination</code> is manageable; otherwise <code>false</code>.
     */
    ...
    /**
     * Initializes the <code>Destination</code> with the properties. If subclasses override, they must call <code>super.initialize()</code>.
     * 
     * @param id
    ...
    /**
     * Verifies that the <code>Destination</code> is in valid state before it is started. If subclasses override, they must call <code>super.validate()</code>.
     */
    @Override
    ...
    /**
     * Starts the destination if its associated <code>Service</code> is started and if the destination is not already running. The default implementation of this method starts the adapter of the
     * destination. If subclasses override, they must call <code>super.start()</code>.
     */
    @Override
    public void start()
    {
    ...
    /**
     * The default implementation of this method stops all of the adapters of the destination. If subclasses override, they must call <code>super.stop()</code>.
     * 
     */
    @Override
    public void stop()
    ...
    /**
     * Creates a <code>ServiceAdapter</code> instance, sets its id, sets it manageable if the <code>Destination</code> that created it is manageable, and set its <code>Destination</code> to the
     * <code>Destination</code> that created it.
     * 
     * In order to use this method, <code>Destination</code> should have an assigned <code>Service</code> and the id provided for the adapter should already be registered with the <code>Service</code>
     * .
     * 
     * @param id
     *            The id of the <code>ServiceAdapter</code>.
     ...
     * 
     * <code>ServiceAdapter</code> needs to be started if the <code>Destination</code> is already running.
     * 
     * @param adapter
     *            The adapter for the destination.
     */
    ...
    /**
     * Used by setAdapter and it removes the old adapter of the destination and adds the new adapter.
     * 
     * @param adapter
     *            The adapter for the destination.
     */
    ...
    /**
     * Used by setAdapter and addAdapter. It removes the current adapter of the destination
     */
    ...
    /**
     * The destination may be not clustered at all, may be clustered for channel failover and destination sharing, or it may be clustered for channel failover and also have a common backend, such as a
     * common database or backend clustered JMS topic. If the destination is clustered and has a common backend to coordinate the cluster, this method returns true; otherwise it return false. Note
     * that this method returns <code>false</code> if the <code>Destination</code> is not runnning.
     * 
     * @return <code>true</code> if the clustered <code>Destination</code> shares a common backend; otherwise <code>false</code>.
     */
    ...
    /**
     * Sets the channel list of the <code>Destination</code>. <code>MessageBroker</code> has to know the channels, otherwise they are not added to the list.
     * 
     * @param ids
     *            List of channel ids.
     */
    ...
    /**
     * The destination may be not clustered at all, may be clustered for channel failover only, or it may be clustered for channel failover and also have shared back ends. If the destination is
     * clustered, regardless of whether or not it relies on a shared back end for cluster configuration, this method returns true. Note that this method returns <code>false</code> if the
     * <code>Destination</code> is not runnning.
     * 
    ...
    /**
     * Sets the id of the <code>Destination</code>. If the <code>Destination</code> has a <code>Service</code> assigned, it also updates the id in the <code>Service</code>.
     * 
     * @param id
     ...
     *            The <code>NetworkSettings</code> of the <code>Destination</code>.
     */
    ...
    /**
     * Sets the <code>Service</code> managing this <code>Destination</code>. Removes the <code>Destination</code> from the old service (if there was one) and adds to the list of destination in the new
     * service.
     * 
     * @param service
    ...
    /**
     * Returns the Java class name for the <code>Service</code> managing this <code>Destination</code>. Returns null if there is no <code>Service</code> assigned to the <code>Destination</code> yet.
     * 
    ...
    /**
     * Returns the <code>SecurityConstraint</code> of the <code>Destination</code>. <code>SecurityConstraint</code> is constructed as the <code>Destination</code> starts up. Therefore, this could
     * return null even if the <code>SecurityConstraint</code> reference is set but <code>Destination</code> is not started yet.
     * 
     ...
     *            The <code>SecurityConstraint</code> of the <code>Destination</code>.
     */
    ...
    /**
     * Sets the <code>SecurityConstraint</code> reference of the <code>Destination</code>. <code>MessageBroker</code> has to know the <code>SecurityConstraint</code> reference. Note that
     * <code>getSecurityConstraint</code> can return null if the reference is set but the <code>Destination</code> is not started yet.
     * 
     * @param ref
     *            <code>SecurityConstraint</code> reference.
     */
    ...
    /**
     * Returns the log category of the <code>Destination</code>. Subclasses can override to provide a more specific log category.
     * 
    ...
    /**
     * Invoked automatically to allow the <code>Destination</code> to setup its corresponding MBean control. Subclasses should override to setup and register their MBean control.
     * 
     * @param service
     *            The <code>Service</code> that manages this <code>Destination</code>.
     */
