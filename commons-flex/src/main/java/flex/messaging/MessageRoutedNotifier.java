...
/**
 * @exclude Supports registration and notification of <tt>MessageRoutedListener</tt>s. An instance of this class is exposed by <tt>FlexContext</tt> while a message is being routed, and once routing of
 *          the message to the outbound messages queues for target clients and registered listeners are notified. This class performs no synchronization because it is only used within the context of a
 *          single Thread, and only during the routing of a single message.
 */
public class MessageRoutedNotifier
{
     ...
     * 
     * @param message
     *            The message that has been routed.
     */
