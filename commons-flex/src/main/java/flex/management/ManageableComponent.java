...
/**
 * An abstract base class that implements the <code>Manageable</code> and <code>FlexComponent</code> interfaces. This is an excellent starting point for a server component that may be instantiated,
 * initialized, started and stopped, as well as exposing an optional management interface via a peer MBean.
 * <p>
 * Support for changing component properties while the component is started should be determined on a per-property basis, and the started property is volatile to ensure consistent reads of the start
 * state of the component across threads. This class performs no synchronization and is not safe for modification by multiple concurrent threads in the absence of external synchronization.
 * </p>
 */
public abstract class ManageableComponent implements Manageable, FlexComponent
{
    ...
    /**
     * Constructs a <code>ManageableComponent</code> instance, specifying whether to enable management. Enabling management will trigger the creation of a peer MBean that exposes the management
    ...
    /**
     * Sets the id of the <code>ManageableComponent</code>. The id cannot be null and it cannot be changed after startup.
     * 
     * @param id
     *            The id of the <code>ManageableComponent</code>.
     */
    ...
    /**
     * Enables or disables management for the component. Management cannot be changed once the component is started and management cannot be <code>true</code> if the parent of the component is not
     * managed.
     * 
     * @param enableManagement
     *            <code>true</code> to enable management, <code>false</code> to disable management.
    ...
    /**
     * Returns the log category of the component. Subclasses must provide an implementation that returns their desired log category.
     * 
    ...
    /**
     * Invoked to initialize the <code>ManageableComponent</code>. This base implementation calls <code>setId()</code> passing the provided id and ignores the properties map argument. Subclasses
     * should call <code>super.initialize()</code>.
     * 
     * @param id
     *            Id of the <code>ManageableComponent</code>.
     * @param properties
     *            Properties for the <code>ManageableComponent</code>.
     */
    @Override
    ...
    /**
     * Convenience method that may be used to generate and throw an Exception for an attempt to set the specified property if the component is started.
     * 
     * @param propertyName
     *            The name of the property being incorrectly assigned; included in the Exception message.
     */
    ...
    /**
     * Convenience method that may be used to generate and throw an Exception for an attempt to assign a null value to a property that requires non-null values.
     * 
     * @param propertyName
     *            The name of the property being incorrectly assigned.
     */
    ...
    /**
     * Invoked from within the <code>stop()</code> method to invalidate the component as part of shutdown. This base implementation sets the valid property to false. Subclasses should call
     * <code>super.invalidate()</code>.
     */
    ...
    /**
     * Hook method invoked from within the <code>start()</code> method to validate that the component is in a startable state. This base implementation validates the component by ensuring it has an id
     * and a parent and then sets the valid property to true. If the component is not in a valid, startable state an Exception is thrown. Subclasses should call <code>super.validate()</code>.
     */
