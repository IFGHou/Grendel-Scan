...
public class Database
{
    ...
    {
		this.file = file;
		Log.debug("Initializing " + file + " database");
        try
		{
		...
		{
			Log.fatal("Failed to load DB driver: " + e.toString());
			System.exit(1);
		}
		...
		{
			Log.fatal("Very weird problem configuring database: " + e.toString(), e);
		}
	}
	...
	public boolean tableExists(String tableName) throws SQLException, Throwable
	{
		...
		{
			Log.fatal("This should never happen: " + e.toString());
			System.exit(1);
		}
	...
	{
		Log.debug("Shutting down " + file + " database");
