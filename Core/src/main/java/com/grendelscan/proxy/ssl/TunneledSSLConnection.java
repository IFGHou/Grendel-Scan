...
public class TunneledSSLConnection extends AbstractHttpServerConnection
{
			...
			throws SSLException, IOException, GeneralSecurityException
	{
		...
		if (destinationHostname == null)
		{
			...
			Log.error("destinationHostname cannot be null", e);
			throw e;
		}
	...
	{
		Log.trace("Initializing SSL for tunnel");
		if (ca == null)
		{
			Log.trace("Getting the static CA");
...
//	}
	...
	{
		int timeout = -1;
		Log.trace("Getting socket timeout");
		if (socket != null)
		{
			try
			{
			...
			{
				Log.warn("Error trying to get socket timeout: " + e.getMessage(), e);
			}
		}
		else
		{
			Log.debug("Socket is null; no socket timeout");
		}
		return timeout;
	}
	...
	public boolean isOpen()
	{
		...
		{
			Log.trace("Gently caught an unexpected close: " + e.toString(), e);
		}
		return open;
	}
	...
	{
		Log.debug("Setting socket timeout to " + timeout);
		assertOpen();
		try
		{
		...
		{
			Log.warn("Error trying to set socket timeout to " + timeout + ": " + e.getMessage(), e);
		}
	}
	...
	public void close()
	{
		...
		{
			Log.trace("Problem closing ssl socket: " + e.toString(), e);
		}
		try
		{
			socket.close();
		...
		{
			Log.trace("Problem closing socket: " + e.toString(), e);
		}
	}
