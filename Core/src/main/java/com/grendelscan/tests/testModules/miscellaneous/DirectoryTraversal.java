...
public class DirectoryTraversal extends TestModule implements ByRequestDataLocationTest
{
	...
	public void testByRequestData(int transactionId, DataReferenceChain chain, int testJobId) throws InterruptedScanException
	{
		...
		ByteData antiTestData = (ByteData) DataContainerUtils.resolveReferenceChain(
		...
		try
		{
					...
					100, Scan.getScanSettings().isParseHtmlDom(), testRequestOptions.followRedirects) < antiPatternThreshold)
			{
				for (String pattern : patterns)
				{
					...
					ByteData testData = (ByteData) DataContainerUtils.resolveReferenceChain(
