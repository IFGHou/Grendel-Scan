...
public class PlaintextHTTP extends TestModule implements ByFileTest
{
	...
	private DatabaseBackedMap<String, Integer>		failedRequestsPerServer;
	...
	public PlaintextHTTP()
	{
		...
		whitelistConfigOption =
				new TextOption("URL regex whitelist", ".*",
						"Each line is a regular expression that will be compared against the base URL.", true, changeHandler);
	...
	{
		return "Tests to see if pages originally requested over HTTPS are accessible over plaintext HTTP. The " +
				"regular expression whitelist will determine which pages are tested. The regex is compared " +
				"to the base URL, but ignores any query. Each "
				+ "line contains a new regex. Any invalid regex formats will be "
				+ "skipped, so be careful. Note that the default is to test all pages.";
	}
	...
	public void testByFile(int transactionID, int testJobId) throws InterruptedScanException
	{
		...
		if (oldFailed >= 5)
		{
			Log.debug("Skipping plaintext http test for " + host + " because failed request threshold met");
			return;
		}
		
		// We're not interested in HTTP, redirects, or non-html responses
		if (!originalTransaction.getRequestWrapper().isSecure() ||
				!shouldBeTested(URIStringUtils.getFileUri(originalTransaction.getRequestWrapper().getURI())) ||
				(originalTransaction.getLogicalResponseCode() != 200) ||
		...
		testTransaction.getRequestWrapper().setURI(
				"http" + testTransaction.getRequestWrapper().getAbsoluteUriString().substring(5), true);
		try
		{
			testTransaction.execute();
			if ((testTransaction.getLogicalResponseCode() == 200)
					&& (HttpResponseScoreUtils.scoreResponseMatch(originalTransaction, testTransaction,
							100, Scan.getScanSettings().isParseHtmlDom(), originalTransaction.getRedirectChildId() > 0) < 95))
			{
...
//						testTransaction.getId());
	...
	private synchronized void report(StandardHttpTransaction transaction)
	{
		...
		else
		{
			...
			String longDescription =
				"At least one page normally transmitted over HTTPS is accessible via plaintext HTTP.<br>" +
						"<br>" +
						"The vulnerable URL(s) are listed below. The links are to the insecure transaction.<br>";
			longDescription += "<a href=\"" + transaction.getSavedUrl() + "\">" + url + "</a><br>\n";
			String impact =
					"If sensitive data (authentication credentials, session IDs, etc) is involved, " +
							"it can be viewed or modified by an attacker that is " +
							"able to view network traffic, or able to launch a man-in-the-middle (MITM) attack.";
			String recomendations =
					"Determine if the transaction in question contains sensitive information. If " +
							"it does, configure the web or application server to only allow the transaction " +
							"over HTTPS.";
			String references = "";
			Finding finding =
					new Finding(null, getName(), FindingSeverity.LOW,
							url,
							title, shortDescription, longDescription, impact, recomendations,
							references);
	...
	private boolean shouldBeTested(String url)
	{
		boolean test = false;
