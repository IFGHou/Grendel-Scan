package com.grendelscan.tests.libraries.SessionIDTesting;
...
{
	private DatabaseBackedMap<String, int[]>	cookieSamples;
	private List<String>		fixationTestPatterns;
	private int					maxCookieRequests;
	private int					maxSessionIDRequests;
	private float				minAcceptableBitChangePercent;
	private int					minAcceptableBitLength;
	private float				minAcceptableNetBitChangePercent;
	private List<String>		notSessionIDs;
	private DatabaseBackedList<String>			testedSessionIDs;
	...
	private SessionID()
	{
		...
		minAcceptableNetBitChangePercent =
				ConfigurationManager.getFloat("session_id_analysis.min_net_bit_change_percentage");
	...
	/**
	 * Looks for SessionIDs in Set-Cookie parameters and the response body.
	 * If there is a cookie and a URL parameter sessionID, only the cookie
	 * location will be returned
	 * 
	 * @param transaction
	 * @return
	 */
	public SessionIDLocation findSessionIDLocation(StandardHttpTransaction transaction)
	{
	...
	public Set<String> findUrlSessionIDs(String htmlBody)
	{
	...
	public void findUrlSessionIDsAndValues(String htmlBody, Map<String, List<String>> sessionIDs)
	{

		for (Pattern sessionIDPattern : Scan.getScanSettings().getReadOnlyKnownSessionIDRegexs())
		{
			Pattern p =
					Pattern.compile("\\b(" + sessionIDPattern.pattern() + ")['\"\\s]*=['\"\\s]*([\\w\\-_]+)",
	...
	public int getBitChangeCount(String stringA, String stringB)
	{
	...
	public int getBitSetCount(int target)
	{
		 ...
		 * April 19, 2006 Don Knuth pointed out to me that this method "was
		 * first published by Peter Wegner in CACM 3 (1960), 322. (Also
	...
	/*
	 * public Set<String> getCookieNames(AbstractHttpTransaction
	 * transaction)
	 * {
	 ...
	 * );
	 * Matcher matcher =
	 * cookiePattern.matcher(transaction.getFirstRequestHeader(
	 * "Cookie").getValue());
	 * while (matcher.find())
	 * {
	...
	public int getNetBitChangeCount(String[] testSessionIDValues)
	{
		int minLength;
		int bitsSet = 0;
	...
	/**
	 * Will return an array of sample cookie transactions. It will also add any
	 * changing cookie values to the list of known session IDs
	 * 
	 * @param transaction
	 *            The transaction to duplicate if no cookie samples are
	 *            available
	 * @param exampleCookie
	 *            The type of cookie being sought. The name, domain and path are
	 *            considered.
	 * @param source
	 ...
	 */
	public StandardHttpTransaction[] getSampleCookieTransactions(StandardHttpTransaction transaction,
			Cookie exampleCookie, String source, int testJobId) throws InterruptedScanException
	{
		...
		if (!cookieSamples.containsKey(exampleCookieFingerprint))
		{
			...
			else
			{
				throw new IllegalArgumentException(
						"No cookies were found, and the transaction didn't have any set-cookie response headers.");
			}
		}
		if (!cookieSamples.containsKey(exampleCookieFingerprint))
		{
		...
		{
			StandardHttpTransaction cookieTransaction =
	 ...
	 */
	public int getSessionIDBitEntropy(List<String> testSessionIDValues)
	{
	 ...
	 * @return -1 if the bits are okay, otherwise the could of bits
	 */
	public int getSessionIDBitEntropy(String[] testSessionIDValues)
	{
		int bitChanges = -1;
		boolean bad = false;
	...
	public Cookie[] getSetCookiesByName(StandardHttpTransaction[] testTransactions, String cookieName) throws InterruptedScanException
	{
		...
		for (int index = 0; index < testTransactions.length; index++)
		{
			...
			if (!foundCookie)
			{
				Log.warn("Cookie not found where one was expected.");
			}
		}
	...
	public boolean isKnownSessionID(String candidate)
	{
		boolean known = false;
	...
	/**
	 * Takes a transaction, looks for set cookies, tests to see if the cookie
	 * values are
	 * changing, then adds them the list of known session IDs if they are
	 * 
	 * @param transaction
	 * @param source
	 *            Execution source
	 * @throws InterruptedException 
	 */
	public void seedSessionIDDatabase(StandardHttpTransaction transaction, String source, int testJobId) throws InterruptedScanException
	{
	...
	public boolean stringContainsURLSessionID(String htmlBody)
	{
	...
	public boolean transactionRequestContainsCookie(StandardHttpTransaction transaction, String cookieName)
	{
		boolean contains = false;
	...
	private String getCookieFingerprint(Cookie cookie)
	{
		return cookie.getName() + cookie.getDomain() + cookie.getPath();
	}

	private int[] getSampleTransactions(StandardHttpTransaction transaction, String source, int testJobId) throws InterruptedScanException
	{
		...
		for (int index = 1; index <= maxCookieRequests; index++)
		{
			...
			/*
			 * The craw client is used because it will not use connection
			 * handling,
			 * causing the connection to be closed after each request. This is
			 * required because some web apps will attach session state with a
			 * TCP
			 * session. Very stupid, but it still happens in the real world.
			 */
			...
			{
				Log.warn("Sample cookie fetcher request unrequstable ("
						+ testTransaction.getRequestWrapper().getAbsoluteUriString() + "): " + e1.toString(), e1);
			}
		}
		...
		{
			/*
			 * If it isn't already known as a session ID, and if it isn't a
			 * a known non-session ID, and if the value is changing
			 */
			if (!isKnownSessionID(cookie.getName())
					&& !notSessionIDs.contains(cookie.getName())
					&& isValueChanging(testTransactions, cookie.getName()))
			{
	...
	private boolean isValueChanging(StandardHttpTransaction[] testTransactions, String cookieName) throws InterruptedScanException
	{
