...
public class UrlEncodingUtils
{
    ...
    public static byte[] encodeAllChars(byte[] bytes)
    {
    	return URLCodec.encodeUrl(ENCODE_ALL_CHARS, bytes);
    }
    
    public static byte[] encodeDefaultChars(byte[] bytes)
    {
    	return URLCodec.encodeUrl(GOOD_URL_CHARS, bytes);
    }
    
    public static String encodeForParam(String string)
    {
    	return new String(URLCodec.encodeUrl(GOOD_URL_PARAM_CHARS, string.getBytes()));
    }

    public static byte[] encodeForParam(byte[] bytes)
    {
	...
	public static byte[] decodeUrl(byte[] bytes) 
	{
	    ...
	    for (int i = 0; i < bytes.length; i++) 
	    {
	        ...
	        else if (b == '%' && i + 2 < bytes.length) 
	        {
	        	...
	        	else // This wasn't a valid hex code, just ignore it
	        	{
	        		buffer.write('%');
