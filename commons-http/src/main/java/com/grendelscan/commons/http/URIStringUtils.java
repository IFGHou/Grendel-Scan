...
{
    private static final Logger LOGGER = LoggerFactory.getLogger(URIStringUtils.class);
    ...
    private final static Pattern urlPattern = Pattern.compile("^(?:" + // beginning of optional hostname gorup
                    "([a-z]+)://" + // scheme
                    "([a-z0-9\\-\\.]++)" + // host
    ...
    private static final Pattern badUriCharPattern = Pattern.compile("((?:[\\x00-\\x20\\x7f-\\xff\\{\\}\\\\\"'`^#|\\[\\]<>\\(\\)])|(?:%(?![0-9a-f]{2})))", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
    ...
    public static void assertAbsoluteHttpAndValid(final String uri) throws URISyntaxException
    {
    ...
    public static String cleanupWhitespace(final String uri)
    {
        if (uri == null)
        {
            return uri;
    ...
    private static String escapeString(final String string, final Pattern p)
    {
     ...
     * @return
     */
    public static String fixBaseUri(final String uri)
    {
        int pos = 0;
        int count = 0;
        ...
        if (count < 3)
        {
            LOGGER.warn("Appended slash to base URI " + uri);
            return uri + "/";
        }
        return uri;
    }

    public static List<String> getAllDirectoryURIs(final String baseUri) throws URISyntaxException
    {
     ...
     * @return
     */
    public static String getDirectory(final String uri) throws URISyntaxException
    {
        return parseUriString(uri)[INDEX_DIRECTORY];
    }

    /**
     * Returns the URI scheme, host, port, and path. No file, query or fragment is included.
     */
    public static String getDirectoryUri(final String uri) throws URISyntaxException
    {
    ...
    public static String getFilename(final String uri) throws URISyntaxException
    {
        return parseUriString(uri)[INDEX_FILE];
    }

    /**
     * Returns the URI scheme, host, port, path & file. No query or fragment is included. Forces a trailing slash if it's only a host with no path
     */
    public static String getFileUri(final String uri)
    {
    ...
    public static String getFirstQueryParameter(final String URI, final String parameterName) throws URISyntaxException
    {
        String value = null;
    ...
    public static String getFragment(final String uri) throws URISyntaxException
    {
        return parseUriString(uri)[INDEX_FRAGMENT];
    }

    public static String getHost(final String uri) throws URISyntaxException
    {
     ...
     */
    public static String getHostUri(final String uri) throws URISyntaxException
    {
     ...
     */
    public static String getHostUriWithoutTrailingSlash(final String uri) throws URISyntaxException
    {
        String components[] = parseUriString(uri);
    ...
    public static int getPort(final String uri) throws URISyntaxException
    {
        String components[] = parseUriString(uri);
    ...
    public static String getQuery(final String uri) throws URISyntaxException
    {
        return parseUriString(uri)[INDEX_QUERY];
    }

    public static List<NameValuePair> getQueryParametersFromQuery(final String query)
    {
    ...
    public static List<NameValuePair> getQueryParametersFromUri(final String URI) throws URISyntaxException
    {
        String query = getQuery(URI);
        return getQueryParametersFromQuery(query);
    }

    public static String getRelativeUri(final String uri) throws URISyntaxException
    {
        String components[] = parseUriString(uri);
    ...
    public static String getScheme(final String uri) throws URISyntaxException
    {
        return parseUriString(uri)[INDEX_SCHEME];
    }

    /**
     * Will return something like http://www.grendel-scan.com:80 It doesn't check to see if this is an absolute URL, so you should check if it could be relative
     */
    public static String getSchemeHostPort(final String uri) throws URISyntaxException
    {
        String components[] = parseUriString(uri);
        return components[INDEX_SCHEME] + "://" + components[INDEX_HOST] + ":" + components[INDEX_PORT];
    }

    public static String getSession(final String uri) throws URISyntaxException
    {
        return parseUriString(uri)[INDEX_SESSION];
    }

    public static boolean isAbsolute(final String uri) throws URISyntaxException
    {
        URI testUri = new URI(uri);
    ...
    /**
     * Checks to see if a URI is usable from a web point of view. Currently, it checks for a scheme of http, https or nothing, and a host if there is a scheme. It also checks for mailto: and
     * javascript: URLs.
     * 
     * @param uri
     * @return
     ...
     */
    public static boolean isUsableUri(final String uri) throws URISyntaxException
    {
        ...
        if (uri != null && !uri.equals(""))
        {
            ...
            {
                String components[] = parseUriString(uri);
                if (components[INDEX_SCHEME].equals("") || (components[INDEX_SCHEME].equalsIgnoreCase("http") || components[INDEX_SCHEME].equalsIgnoreCase("https")) && !components[INDEX_HOST].equals(""))
                {
                    usable = true;
                }
            }
    ...
    public static void main(final String[] args)
    {
        ...
        printComponents("http://www.example.com/classics/index.html" + "?a=1&b=2&c=asdf;fdsa");
    ...
    /**
     * Takes the URI and adds a port (if the scheme is known), sorts the parameter order, removes duplicate paramater names (if this breaks your stuff, follow the standard (RTFRFC)), removes extra
     * ampersands (&) in the query, sets the scheme and host to all lower case (path & query can be case sensitive depending on OS & web server), and removes the fragment (since it isn't relilvent for
     * requests)
     * 
     * @param uri
     * @return
     * @throws URISyntaxException
     */
    public static String normalizeUri(final String uri) throws URISyntaxException
    {
        String components[] = parseUriString(uri);
    ...
    public static String normalizeUriQuery(final String query)
    {
    ...
    private static String[] parseUriString(final String uri) throws URISyntaxException
    {
    ...
    private static void printComponents(final String uri)
    {
        String[] comps;
        try
        {
    ...
    private static String reconstituteUri(final String[] components)
    {
        StringBuilder sb = new StringBuilder();
    ...
    public static String removeDoubleSlashesFromDir(final String uri)
    {
        if (uri == null)
        {
            return uri;
    ...
    public static String removeQueryParameter(final String uri, final String parameterName) throws URISyntaxException
    {
        String newUri = uri;
    ...
    public static String replaceFilename(final String uri, final String newFilename) throws URISyntaxException
    {
        String[] components = parseUriString(uri);
    ...
    public static String replaceQuery(final String uri, final String newQuery) throws URISyntaxException
    {
        String[] components = parseUriString(uri);
    ...
    public static String replaceQueryParameter(final String uri, final String parameterName, final String parameterValue) throws URISyntaxException
    {
        String newUri = uri;
        String query = getQuery(newUri);
        if (StringUtils.notEmpty(query))
    ...
    public static String replaceSession(final String uri, final String newSession) throws URISyntaxException
    {
        String[] components = parseUriString(uri);
    ...
    public static String urlEncode(final Iterable<NameValuePair> pairs)
    {
        boolean first = true;
    ...
    public static String urlEncode(final NameValuePair[] pairs)
    {
        return urlEncode(Arrays.asList(pairs));
    }

    public static boolean validateURISyntax(final String URI, final boolean mustBeAbsolute)
    {
        boolean good = true;
        try
        {
