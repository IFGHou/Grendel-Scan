...
import com.grendelscan.commons.http.wrappers.HttpHeadersWrapper;
import com.grendelscan.commons.http.wrappers.HttpMessageWrapper;
...
public class HttpUtils
{
    ...
    /**
     * It might seem strange to say that a 302 (or other redirect) isn't a file found. Keep in mind two things. First, many file not found messages are in redirects. Second, the redirect's destination
     * will be tested as a file found (if it is found).
     * 
     * @param httpStatusCode
     * @return
     */
    public static boolean fileExists(final int httpStatusCode)
    {
        boolean exists = false;
        // 12/10/10 - changed this to include all 3xx codes
        if (httpStatusCode >= 100 && httpStatusCode < 400 || httpStatusCode >= 401 && httpStatusCode <= 403)
        {
            exists = true;
        }
        return exists;
    ...
    public static List<String> getAllQueryParameterNames(final StandardHttpTransaction transaction)
    {
    ...
    public static Cookie getSetCookie(final StandardHttpTransaction transaction, final String cookieName)
    {
        for (Header setCookie : transaction.getResponseWrapper().getHeaders().getHeaders("Set-Cookie"))
        {
            ...
            {
                LOGGER.error("Problem with set-cookie header in HttpUtils.getSetCookies: " + e.toString(), e);
            }
        }
        return null;
    }

    public static Set<String> getSetCookieNames(final StandardHttpTransaction transaction)
    {
    ...
    public static List<SerializableBasicCookie> getSetCookies(final HttpTransactionFields transaction)
    {
    ...
    public static StandardHttpTransaction parseIntoHttpRequest(final TransactionSource source, final byte[] rawRequest, final int testJobId) throws HttpFormatException, URISyntaxException
    {
    ...
    private static void parseMessage(final HttpMessageWrapper wrapper, final HttpTransactionByteInputBuffer inputBuffer, final BasicLineParser lineParser) throws IOException, HttpException
    {
    ...
    public static HttpRequestWrapper parseRequest(final byte[] data) throws IOException, HttpException
    {
    ...
    public static HttpResponseWrapper parseResponse(final byte[] data) throws IOException, HttpException
    {
        HttpTransactionByteInputBuffer inputBuffer = new HttpTransactionByteInputBuffer(data);
        CharArrayBuffer lineBuf = new CharArrayBuffer(128);
