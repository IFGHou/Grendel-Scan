...
public interface HTMLDocument extends Document
{
	...
	/**
	 * A collection of all the anchor (<code>A</code>) elements in a
	 * document with a value for the <code>name</code> attribute. For reasons
	 * of backward compatibility, the returned set of anchors only contains
	 * those anchors created with the <code>name</code> attribute, not those
	 * created with the <code>id</code> attribute. Note that in [<a
	 ...
	 * of the current frame or document. If no path attribute is specified, then
	 ...
	 * documents, this method returns the (possibly empty) collection of
	 * elements whose <code>name</code> value is given by
	 ...
	 *                href='http://www.ietf.org/rfc/rfc2965.txt'>IETF RFC 2965</a>].
	 */
	 ...
	 * produce a document which is not necessarily driven by a DTD and therefore
