...
/**
 * The <code>HtmlParser</code> class is an HTML DOM parser. This parser provides the functionality for the standard DOM parser implementation {@link org.cobra_grendel.html.parser.DocumentBuilderImpl}.
 * This parser class may be used directly when a different DOM implementation is preferred.
 */
public class HtmlParser
{
    /**
     * A node <code>UserData</code> key used to tell nodes that their content may be about to be modified. Elements could use this to temporarily suspend notifications. The value set will be either
    ...
    {
        return new LocatorImpl(publicId, systemId, lineNumber, columnNumber);
    }
    ...
    /**
     * This method may be used when the DOM should be built under a given node, such as when <code>innerHTML</code> is used in Javascript.
     * 
     * @param reader
    ...
    /**
     * Parses HTML given by a <code>Reader</code>. This method appends nodes to the document provided to the parser.
     * 
     * @param reader
    ...
    /**
     * Assumes that the content is completely made up of text, and parses until an ending tag is found.
     * 
     * @param parent
     * @param reader
     * @param tagName
     ...
     * @throws IOException
     */
    private final int parseForEndTag(final Node parent, final LineNumberReader reader, final String tagName, final boolean addTextNode, final boolean decodeEntities) throws IOException, SAXException
    {
        Document doc = document;
        int intCh;
                    ...
                    if (ch == '/')
                    {
                        ...
                        while ((intCh = reader.read()) != -1)
                        {
                            ...
                            if (ch == '>')
                            {
                                ...
                                {
                                    justReadTagBegin = false;
                                    justReadTagEnd = true;
                                    justReadEmptyElement = false;
                                    normalLastTag = thisTag.toUpperCase();
                                    if (addTextNode)
                                    {
                                        if (decodeEntities)
                                        {
     ...
     * @throws SAXException
     */
    private final int parseToken(final Node parent, final LineNumberReader reader, final Set stopTags, final LinkedList ancestors) throws IOException, StopException, SAXException
    {
        Document doc = document;
        StringBuffer textSb = readUpToTagBegin(reader);
        if (textSb == null)
        {
            return TOKEN_EOD;
        }
        if (textSb.length() != 0)
        {
            ...
            StringBuffer decText = entityDecode(textSb);
            ...
            try
            {
                if (tag.startsWith("!"))
                {
                    if ("!--".equals(tag))
                    {
                        ...
                        StringBuffer comment = passEndOfComment(reader);
                        StringBuffer decText = entityDecode(comment);
                                    ...
                                    for (;;)
                                    {
                                        try
                                        {
                                            ...
                                            if (einfo != null && einfo.noScriptElement)
                                            {
                                                ...
                                                {
                                                    token = parseForEndTag(parent, reader, tag, false, einfo.decodeEntities);
                                                }
                                                else
                                                {
                                            ...
                                            {
                                                token = childrenOk ? parseToken(element, reader, newStopSet, ancestors) : parseForEndTag(element, reader, tag, true, einfo.decodeEntities);
                                            }
    ...
    private final boolean readAttribute(final LineNumberReader reader, final Element element) throws IOException, SAXException
    {
        ...
        for (;;)
        {
            ...
            else if (ch == '>')
            {
                ...
                justReadTagBegin = false;
                justReadTagEnd = true;
                justReadEmptyElement = lastCharSlash;
                return false;
            }
            else if (ch == '/')
            {
        ...
        for (;;)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (lastCharSlash)
                    {
                        attributeValue.append('/'); // In case the value started with a slash, and no quote
                    }
        ...
        for (;;)
        {
            ...
            if (openQuote != -1 && ch == openQuote)
            {
                ...
                if (attributeName != null)
                {
                    ...
                    {
                        StringBuffer actualAttributeValue = entityDecode(attributeValue);
     ...
     * @return
     */
    private final String readTag(final LineNumberReader reader) throws IOException
    {
        StringBuffer sb = new StringBuffer();
        int chInt;
        chInt = reader.read();
    ...
    /**
     * Reads text until the beginning of the next tag. Leaves the reader offset past the opening angle bracket. Returns null only on EOF.
     */
