...
public class HTMLBaseFontElementImpl extends HTMLAbstractUIElement implements HTMLBaseFontElement
{
    ...
    public HTMLBaseFontElementImpl(final String name, final int transactionId)
    {
    ...
    /*
     * protected RenderState createRenderState(RenderState prevRenderState) { String face = this.getAttribute("face"); String size = this.getAttribute("size"); String color =
     * this.getAttribute("color"); if(face != null) { prevRenderState = new FontNameRenderState(prevRenderState, face); } if(size != null) { int fontNumber = HtmlValues.getFontNumberOldStyle(size,
     * prevRenderState); float fontSize = HtmlValues.getFontSize(fontNumber); prevRenderState = new FontSizeRenderState(prevRenderState, fontSize); prevRenderState = new
     * BaseFontRenderState(prevRenderState, fontNumber); } if(color != null) { prevRenderState = new ColorRenderState(prevRenderState, ColorFactory.getInstance().getColor(color)); } return
     * prevRenderState; }
     */
}
