...
public class HTMLTableElementImpl extends HTMLAbstractUIElement implements HTMLTableElement
{
    ...
    public HTMLTableElementImpl(final int transactionId)
    {
        super("TABLE", transactionId);
    }

    public HTMLTableElementImpl(final String name, final int transactionId)
    {
     ...
     */
    /*
     * public HtmlLength getHeightLength(int availHeight) { try { CSS2PropertiesImpl props = this.getCurrentStyle(); String heightText = props == null ? null : props.getHeight(); if(heightText ==
     * null) { return new HtmlLength(this.getAttribute("height")); } else { return new HtmlLength(HtmlValues.getPixelSize(heightText, this.getRenderState(), 0, availHeight)); } } catch(Exception err)
     * { return null; } }
     */
    /*
     * (non-Javadoc)
     ...
     */
    /*
     * public HtmlLength getWidthLength(int availWidth) { try { CSS2PropertiesImpl props = this.getCurrentStyle(); String widthText = props == null ? null : props.getWidth(); if(widthText == null) {
     * return new HtmlLength(this.getAttribute("width")); } else { return new HtmlLength(HtmlValues.getPixelSize(widthText, this.getRenderState(), 0, availWidth)); } } catch(Exception err) { return
     * null; } }
     */
    @Override
    ...
    /**
     * Inserts a row at the index given. If <code>index</code> is <code>-1</code>, the row is appended as the last row.
     */
    @Override
