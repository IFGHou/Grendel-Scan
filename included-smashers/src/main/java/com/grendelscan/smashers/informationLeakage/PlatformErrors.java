...
import com.grendelscan.smashers.utils.platformErrorMessages.PlatformErrorMessages;

public class PlatformErrors extends AbstractSmasher implements AllTransactionsTest
{
    ...
    {
        return "Searches all responses for platform error messages (.Net, PHP, Cold Fusion, MySQL, Oracle, etc). " + "Only the first error message in each response will be reported, so you may want to manually " + "review the entire response.\n" + "\n"
                        + "An optional list of regular expressions that define platform error messages can be provided. " + "The expression is matched against any part of the response, so \"error\" will work (you don't "
    ...
    public synchronized void logFinding(final StandardHttpTransaction transaction)
    {
        try
        {
            if (finding == null)
        ...
        catch (DataNotFoundException e)
        {
            ...
            String longDesc = "Some platform error messages were found in a response. The transactions" + " are listed in the table below:<br><br>" + "\n<table><tr style=\"font-weight: bold\"><td>Transaction</td><td>Match pattern</td></tr>\n";

            String impact = "The impact will vary depending on the nature of the error.";
            String recomendations = "Each result should be individually investigated. The error message " + "may provide a hint about the internal structure of the application, or attacks that " + "can be launched.";
        ...
        desc += "<tr><td>" + HtmlUtils.makeLink(transaction.getSavedUrl(), String.valueOf(transaction.getId())) + "</td><td>" + HtmlUtils.escapeHTML(PlatformErrorMessages.getInstance().getReadOnlyMessageHits().get(transaction.getId())) + "</td></tr>\n";

        finding.setLongDescription(desc);
    }

    @Override
    public void testAllTransactions(final int transactionID, final int testJobId)
    {
