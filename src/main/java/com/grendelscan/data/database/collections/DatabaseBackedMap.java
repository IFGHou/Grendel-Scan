...
public class DatabaseBackedMap<K, V> extends DatabaseBackedCollection implements DatabaseUser, Map<K, V>
{
	...
	{
		Log.debug("Initializing database for database-backed map: " + collectionName);
		try
		{
		...
		{
			Log.info("No data found in " + collectionName + " collection. This may not be a problem.", e);
		}
		catch (Throwable e)
		{
			Log.fatal("Problem with creating database for " + uniqueName + " database backed map: " + e.toString(), e);
			System.exit(1);
		}
	}
	...
	private void removeFromDb(Object name)
	{
		...
		{
			Log.error("Problem deleting from " + collectionName + ": " + e.toString(), e);
		}
	}
