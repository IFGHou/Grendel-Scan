...
//	                "trigger_connect" };
	...
	public AmfCommandMessage(String name, boolean externalizable, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId, AmfActionMessageRoot amfRoot)
	{
		super(name, AmfDataType.kAmfCommandMessage, externalizable, parent, reference, transactionId);
		AmfOperation operation = new AmfOperation(OPERATION_PROPERTY_NAME, CommandMessageTypeEnum.UNKNOWN_OPERATION, this, NameOrValueReference.VALUE, transactionId);
		addFixedField(OPERATION_PROPERTY_NAME, operation, true);
	}
	
	public AmfCommandMessage(String name, CommandMessage message, String className, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		super(name, message, AmfDataType.kAmfCommandMessage, className, parent, reference, transactionId);
		AmfOperation operation = new AmfOperation(OPERATION_PROPERTY_NAME, CommandMessageTypeEnum.getByValue(message.getOperation()), this, NameOrValueReference.VALUE, transactionId);
		addFixedField(OPERATION_PROPERTY_NAME, operation, true);
	}
