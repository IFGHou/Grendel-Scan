...
public class AmfActionMessageHeader extends AbstractAmfDataContainer<AmfActionMessageHeaderComponentReference>
{
	...
	public AmfActionMessageHeader(MessageHeader header, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId, AmfActionMessageRoot amfRoot)
	{
		super(header.getName(), AmfDataType.kActionMessageHeader, parent, false, reference, transactionId);
		mustUnderstand = new AmfPrimitiveData("Must understand", header.getMustUnderstand(), this, AmfActionMessageHeaderComponentReference.MUST_UNDERSTAND, transactionId);
		mustUnderstand.setTypeLocked(true);
		data = AmfUtils.parseAmfData(header.getData(), parent, AmfActionMessageHeaderComponentReference.DATA, transactionId);
	}
	
	public AmfActionMessageHeader(String name, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId, AmfActionMessageRoot amfRoot)
	{
		super(name, AmfDataType.kActionMessageHeader, parent, false, reference, transactionId);
		mustUnderstand = new AmfPrimitiveData("Must understand", false, this, AmfActionMessageHeaderComponentReference.MUST_UNDERSTAND, transactionId);
		mustUnderstand.setTypeLocked(true);
	}
