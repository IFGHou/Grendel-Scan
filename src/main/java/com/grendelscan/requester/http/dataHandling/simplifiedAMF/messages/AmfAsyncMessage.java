...
public class AmfAsyncMessage extends AmfAbstractMessage
{
	...
	public AmfAsyncMessage(String name, boolean externalizable, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		super(name, AmfDataType.kAmfAsyncMessage, externalizable, parent, reference, transactionId);
		initCorrelationId("");
	}
	
	protected AmfAsyncMessage(String name, AbstractMessage message, AmfDataType type, String className, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		super(name, message, type, className, parent, reference, transactionId);
		initCorrelationId("");
	}
	
	public AmfAsyncMessage(String name, AsyncMessage message, String className, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		super(name, message, AmfDataType.kAmfAsyncMessage, className, parent, reference, transactionId);
		initCorrelationId(message.getCorrelationId());
		
	}
	
	protected AmfAsyncMessage(String name, AmfDataType type, boolean externalizable, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		super(name, type, externalizable, parent, reference, transactionId);
		initCorrelationId("");
	}
	
	private void initCorrelationId(String value)
	{
		addFixedField(correlationIdName, new AmfPrimitiveData(correlationIdName, value.getBytes(), this, NameOrValueReference.VALUE, getTransactionId()), true);
	}
	...
	public void writeExternal(AmfOutputStream outputStream) throws IOException
	{
		...
		{
			AmfByteArray aba = new AmfByteArray("Correlation ID", correlationIdBytes, this, NameOrValueReference.VALUE, getTransactionId());
