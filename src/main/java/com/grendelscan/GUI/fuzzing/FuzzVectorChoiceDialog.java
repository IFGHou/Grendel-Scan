...
import com.grendelscan.GUI.fuzzing.vectorComposites.FuzzVectorComposite;
import com.grendelscan.GUI.fuzzing.vectorComposites.NumericSequenceComposite;
import com.grendelscan.GUI.fuzzing.vectorComposites.PresetStringComposite;
import com.grendelscan.GUI.fuzzing.vectorComposites.StringSequenceComposite;
...
*/
public class FuzzVectorChoiceDialog extends org.eclipse.swt.widgets.Dialog 
{
	private Button cancelButton;
	private Button okButton;
	private Group fuzzVectorGroup;
	Combo vectorTypeCombo;
	Composite currentFuzzComposite;
	private FormData fuzzCompositeLayout;
	FuzzVector fuzzVector;
	Shell dialogShell;

		
	public static FuzzVector getFuzzVector(Shell shell, FuzzVector oldVector) 
	{
		FuzzVectorChoiceDialog inst = new FuzzVectorChoiceDialog(shell, SWT.NULL, oldVector);
		return inst.fuzzVector;
	}

	public FuzzVectorChoiceDialog(Shell parent, int style, FuzzVector oldVector) 
	{
		super(parent, style);
		initGUI(oldVector);
	}
	...
	private void initGUI(FuzzVector oldVector) 
	{
			...
			dialogComposite.setLayout(new FormLayout());
			{
				...
				vectorTypeCombo.addModifyListener(new ModifyListener() 
				{
					...
					{
						Composite composite = (Composite) vectorTypeCombo.getData(vectorTypeCombo.getText());
			...
			{
				fuzzVectorGroup = new Group(dialogComposite, SWT.NONE);
