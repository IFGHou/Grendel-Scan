...
import com.grendelscan.GUI.Verifiers.EnforceHexOnly;
...
*/
public class HexViewComposite extends Composite implements ParsedEntityComposite {
	Table hexTable;
	TableEditor editor;
	...
	public HexViewComposite(org.eclipse.swt.widgets.Composite parent, int style, boolean editable) 
	{
		super(parent, style);
	...
	private void initGUI() 
	{
		SWTResourceManager.registerResourceUser(this);
			...
			this.setLayout(thisLayout);
			{
				...
				hexTable = new Table(this, SWT.NONE);
				hexTable.setHeaderVisible(true);
				hexTable.setLinesVisible(true);
				hexTable.setFont(SWTResourceManager.getFont("Courier New", 9, SWT.BOLD, false, false));
					...
					hexTable.addListener(SWT.MouseDoubleClick, new Listener()
					{
						...
						public void handleEvent(Event event)
						{
							...
							while (index < hexTable.getItemCount())
							{
								...
								final TableItem item = hexTable.getItem(index);
								...
								for (int i = 1; i < 17; i++)
								{
									...
									{
										final int column = i;
										final Text text = new Text(hexTable, SWT.NONE);
										final boolean hex;
										if (i > 0 && i < 17)
										{
											hex = true;
										...
										text.setFont(SWTResourceManager.getFont("Courier New", 9, SWT.BOLD, false, false));
				...
				{
					positionColumn = new TableColumn(hexTable, SWT.NONE);
				...
				{
					textColumn = new TableColumn(hexTable, SWT.NONE);
	...
	private void populateTable(byte[] data)
	{
		...
		{
			TableItem item = new TableItem(hexTable, SWT.NONE);
			item.setFont(SWTResourceManager.getFont("Courier New", 9, SWT.BOLD, false, false));
	...
	private TableColumn makeHexColumn(Table parent, String name)
	{
		TableColumn hexColumn = new TableColumn(parent, SWT.NONE);
