...
public class ParsedResponseComposite extends ScrolledComposite {
	...
	private Group responseBodyGroup;
	private ParsedHttpHeaderComposite httpResponseHeaderList;
	private Group httpHeadersGroup;
	private Text httpMessageTextBox;
	private Text codeTextBox;
	private Text versionTextBox;
	private Label messageLabel;
	private boolean editable;
	private Composite responseHeadComposite;
	...
	public ParsedResponseComposite(Composite parent, int style, boolean editable)
	{
	...
	public void updateData(HttpResponseWrapper response)
	{
		if (response == null)
		{
			clearData();
	...
	private void initGUI() 
	{
			...
			{
				responseHeadComposite = new Composite(mainSash, SWT.NONE);
				...
				{
					versionLabel = new Label(responseHeadComposite, SWT.NONE);
				...
				}
				{
					...
					versionTextBox = new Text(responseHeadComposite, SWT.BORDER);
					versionTextBox.setEditable(editable);
					versionTextBox.setLayoutData(versionTextBoxLData);
				}
				{
					codeLabel = new Label(responseHeadComposite, SWT.NONE);
	...
	
				{
					...
					codeTextBox = new Text(responseHeadComposite, SWT.BORDER);
				...
				{
					messageLabel = new Label(responseHeadComposite, SWT.NONE);
				...
				}
				{
					...
					httpMessageTextBox = new Text(responseHeadComposite, SWT.BORDER);
			...
			{
				httpHeadersGroup = new Group(responseHeadComposite, SWT.NONE);
			...
			{
				responseBodyGroup = new Group(mainSash, SWT.NONE);
	...
	public byte[] getBytes()
	{
		...
		{
			Log.error("Problem getting bytes for response: " + e.toString(), e);
		}
...
//	}
	...
	public HttpResponseWrapper makeHttpResponse() throws HttpFormatException
	{
		...
		responseWrapper.setStatusLine(new SerializableStatusLine(
				new ProtocolVersion(m.group(1), Integer.valueOf(m.group(2)), Integer.valueOf(m.group(3))), 
				Integer.valueOf(this.codeTextBox.getText()), 
				httpMessageTextBox.getText()));
