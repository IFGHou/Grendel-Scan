...
{
	public static AbstractAmfData parseAmfArray(Object[] objects, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		...
		if (objects.length > 0)
		{
			if (objects[0] instanceof Byte)
			{
				...
				array = new AmfByteArray("", bytes, parent, reference, transactionId);
			}
			else
			{
				array = getArray(objects[0], parent, reference, transactionId);
				for (Object object: objects)
				{
					((AmfPrimitiveArray)array).addChild(parseAmfData(object, parent, reference, transactionId));
				}
			}
		}
		else
		{
			array = new AmfObjectArray("", parent, reference, transactionId);
		}
		return array;
	}
	
	public static AbstractAmfData parseAmfCollection(Collection<Object> collection, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		return parseAmfArray(collection.toArray(), parent, reference, transactionId);
	}

	private static AmfPrimitiveArray getArray(Object o, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		if (o instanceof Boolean)
		{
			return new AmfBooleanArray("", parent, reference, transactionId);
		}
		else if (o instanceof Integer || o instanceof Short || o instanceof Long)
		{
			return new AmfIntArray("", parent, reference, transactionId);
		}
		else
		{
			return new AmfObjectArray("", parent, reference, transactionId);
		}
	}
	
	
	public static AbstractAmfData parseAmfData(Object o, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		AbstractAmfData data = null;
		if (o == null)
		{
			data = new AmfPrimitiveData("", AmfDataType.kNull, new byte[0], parent, reference, transactionId);
		}
		else if ((o instanceof String) || (o instanceof Character))
		{
			data = new AmfPrimitiveData("", o.toString().getBytes(), parent, reference, transactionId);
		}
		else if (o instanceof Number)
		{
			if ((o instanceof Integer) || (o instanceof Short) || (o instanceof Byte))
			{
				...
				data = new AmfPrimitiveData("", i, parent, reference, transactionId);
			}
			...
			else
			{
				...
				data = new AmfPrimitiveData("", d, parent, reference, transactionId);
			}
		}
		else if (o instanceof Boolean)
		{
			data = new AmfPrimitiveData("", ((Boolean) o).booleanValue(), parent, reference, transactionId);
		}
		// We have a complex type...
		else if (o instanceof Date)
		{
			data = new AmfDateTime("", ((Date) o).getTime(), parent, reference, transactionId);
		}
		else if (o instanceof Calendar)
		{
			data = new AmfDateTime("", ((Calendar) o).getTimeInMillis(), parent, reference, transactionId);
		}
		else if (o instanceof Document)
		{
			String xml = HtmlNodeWriter.write((Document) o, true, null);
			data = new AmfPrimitiveData("", AmfDataType.kAvmPlusXml, xml.getBytes(), parent, reference, transactionId);
		}
		else if (o instanceof ASObject)
		{
			data = new AmfASObject("", (ASObject) o, parent, reference, transactionId);
		}
		else if ((o instanceof Map) && !(o instanceof ASObject))
		{
			data = parseAmfMap((Map<Object, Object>) o, parent, reference, transactionId);
		}
		else if (o instanceof Collection)
		{
			data = parseAmfCollection((Collection<Object>) o, parent, reference, transactionId);
		}
		else if (o instanceof AmfServerSideObject)
		{
			AmfServerSideObject sso = (AmfServerSideObject) o;
			data = sso;
			sso.initialize(parent, reference, transactionId);
		}
		else if (o instanceof CommandMessage)
		{
			data = new AmfCommandMessage("", (CommandMessage) o, o.getClass().getCanonicalName(), parent, reference, transactionId);
		}
		else if (o instanceof ErrorMessage)
		{
			data = new AmfErrorMessage("", (ErrorMessage) o, parent, reference, transactionId);
		}
		else if (o instanceof AcknowledgeMessage)
		{
			data = new AmfAcknowledgeMessage("", (AcknowledgeMessage) o, o.getClass().getCanonicalName(), parent, reference, transactionId);
		}
		else if (o instanceof AsyncMessage)
		{
			data = new AmfAsyncMessage("", (AsyncMessage) o, o.getClass().getCanonicalName(), parent, reference, transactionId);
		}
		else if (o instanceof RemotingMessage)
		{
			data = new AmfRemotingMessage("", (RemotingMessage) o, parent, reference, transactionId);
		}
		else if (o.getClass().isArray())
		{
			data = parseAmfArray((Object[]) o, parent, reference, transactionId);
		}
		else
		{
			data = new AmfServerSideObject(o, parent, reference, transactionId);
//			Log.warn("Problem with unknown data type in AmfUtils: " + o.getClass().getCanonicalName() + "\n" + o.toString());
		}
		return data;
	}
	
	public static AmfAssociativeArrayData parseAmfMap(Map<Object, Object> map, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId)
	{
		AmfAssociativeArrayData data = new AmfAssociativeArrayData("", parent, reference, transactionId);
		for (Object o: map.keySet())
		{
			String key = o.toString();
			data.putChild(key, parseAmfData(map.get(o), parent, reference, transactionId));
		}
    ...
    public static AbstractAmfData amfFactory(AmfDataType type, AbstractAmfDataContainer<?> parent, DataReference reference, int transactionId, AmfActionMessageRoot amfRoot) throws IllegalArgumentException
    {
    	switch(type)
    	{
    		case kActionMessageHeader:
    			return new AmfActionMessageHeader("", parent, reference, transactionId, amfRoot);
    		case kAmfAsyncMessage:
    			return new AmfAsyncMessage("", false, parent, reference, transactionId);
    		case kAmfBody:
    			return new AmfBody("", "", parent, reference, transactionId, amfRoot);
    		case kAmfCommandMessage:
    			return new AmfCommandMessage("", false, parent, reference, transactionId, amfRoot);
    		case kAmfMessage:
    			throw new IllegalArgumentException("Can't instantiate AmfMessage (it's abstract)");
    		case kAmfMessageBodies:
    			return new AmfMessageBodies(parent, reference, transactionId, amfRoot);
    		case kAmfMessageHeaders:
    			return new AmfMessageHeaders(parent, reference, transactionId, amfRoot);
    		case kAmfMessageRoot:
    			return new AmfActionMessageRoot(3, transactionId);
    		case kASObject:
    			return new AmfASObject("", parent, reference, transactionId);
    		case kAssociativeArray:
    			return new AmfAssociativeArrayData("", parent, reference, transactionId);
    		case kAvmPlusXml:
    		case kString:
    		case kXML:
    		case kTrue:
    		...
    		case kUndefined:
    		case kFalse:
    			return new AmfPrimitiveData("", type, new byte[0], parent, reference, transactionId);
    		case kBooleanArray:
    			return new AmfBooleanArray("", parent, reference, transactionId);
    		case kByteArray:
    			return new AmfByteArray("", parent, reference, transactionId);
    		case kCommandType:
    			return new AmfOperation("", parent, reference, transactionId);
    		case kDate:
    			return new AmfDateTime("", 0, parent, reference, transactionId);
    		case kDoubleArray:
    			return new AmfDoubleArray("", parent, reference, transactionId, amfRoot);
    		case kIntArray:
    			return new AmfIntArray("", parent, reference, transactionId);
    		case kObjectArray:
    			return new AmfObjectArray("", parent, reference, transactionId);
    		case kServerSideObject:
    			return new AmfServerSideObject("", parent, reference, transactionId);
    		case kAmfRemotingMessage:
    			return new AmfRemotingMessage("", parent, reference, transactionId);
    		default:
