...
import com.grendelscan.utils.dataFormating.encoding.HexEncodingUtils;
import com.grendelscan.utils.dataFormating.encoding.HtmlEncodingUtils;
...
public class DataFormatUtils
{
	...
	private final static Pattern BASE64_PRETTY_PATTERN = Pattern.compile(
									"^(?:[a-zA-Z0-9+/]{64,76}={0,2}\\x0d?\\x0a)" + // has to be at least one full line
									"[a-zA-Z0-9+/]{0,76}+={0,2}" +
									"\\x0d?\\x0a?$" // last line doesn't have to end with CRLF
									);
	...
	public static byte[] encodeData(byte[] data, DataFormatType format, DataFormatOptions options) throws DataFormatException 
	{
		if (!format.isEncodeable())
		{
			throw new NotImplementedException(format.toString() + " is not implemented for decoding; it is a basic data format.");
	...
	private static float getWordPercentage(String data)
	{
	...
	private static String stripWords(String data)
	{
