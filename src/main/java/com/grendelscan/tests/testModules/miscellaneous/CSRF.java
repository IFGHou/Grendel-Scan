...
public class CSRF extends TestModule implements ByHtmlFormTest
{
...
//	private final Set<String>			testedForms;
	private TextOption					whitelistConfigOption;
	...
	public CSRF()
	{
		...
		whitelistConfigOption =
				new TextOption("Action regex whitelist", ".*",
						"Each line is a regular expression that will be compared against the form's action.", true, changeHandler);
	...
	{
		return "Tests for cross-site request forgery (CSRF) vulnerabilities in queries "
				+ "based on HTML forms. Some queries forms (e.g. a search function) don't "
				+ "need to be resistant to CSRF. The regular expression white-list is "
				+ "tested against the absolute URL version of the form's action. Each "
				+ "line contains a new regex. Any invalid regex formats will be "
				+ "skipped, so be careful. Note that the default is to test all forms.";
	}
	...
	private String getAbsoluteAction(HTMLElement element)
	{
		...
		{
			Log.error("Invalid action format: " + e.toString(), e);
		}
		return uri;
	}

	private String getFormProfile(HTMLElement element, boolean useValues)
	{
	...
	private String[] getParamNames(HTMLElement element)
	{
	...
	private void record(StandardHttpTransaction originalTransaction, String action, String[] params)
	{
		...
		String description =
								"<b>Action</b>: "
										...
										+ query
										+ "<br>"
										+ "<b>Original transaction</b>: "
										+ HtmlUtils.makeLink(originalTransaction.getSavedUrl(),
												String.valueOf(originalTransaction.getId())) + "<br>"
										+ "<br>\n";
		logFinding(description);
	}

	private synchronized void logFinding(String findingDescription)
	{
		if(finding ==null)
		{
			...
			String longDescription =
					"One or more cross-site request forgery (CSRF) vulnerabilities may have been identified. "
							+ "CSRF allows an attacker to force a user to execute arbitrary commands "
							+ "against the vulnerable website. This is possible when the structure of the "
							+ "command is predictable. If the command can be requested as a GET, then a "
							+ "simple IMG tag on an attack website can force the browser to send the "
							+ "command. A POST request can be sent using some simple JavaScript. The "
							+ "browser will send any cookies or authentication credentials associated "
							+ "with the targeted attack, because it has no way of knowing that the "
							+ "request was not intentionally executed by the user.<br>" + "<br>"
							+ "A list of queries that appear to be vulnerable to CSRF is below. "
							+ "A specific form may be found on multiple pages, but was only tested once.<br>"
							+ "<br>";
			...
			String recomendations =
					"If this query is used to execute a sensitive function "
							+ "(e.g. creating a new user), then CSRF is a significant vulnerability. It "
							+ "can be prevented by using a random token as a query parameter. Before "
							+ "executing the command, the application should confirm that the token sent by "
							+ "the browser matches the original token sent by the application. Since an "
							+ "attacker couldn't know the random token (without a secondary vulnerability "
							+ "like XSS), a CSRF attack is not possible.";
			String references = HtmlUtils.makeLink("http://www.owasp.org/index.php/Cross-Site_Request_Forgery");
			finding =
					new Finding(null, getName(), FindingSeverity.HIGH, "See description", title,
							shortDescription, longDescription, impact, recomendations,
							references);
	...
	private boolean shouldBeTested(String action)
	{
		boolean test = false;
