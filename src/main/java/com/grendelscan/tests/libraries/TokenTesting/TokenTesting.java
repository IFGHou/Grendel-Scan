...
/**
 * This is used to track when tokens are submitted in a query to probe where
 * they
 * end up. It is mostly useful for XSS and HTTP response splitting testing.
 * 
 * @author David Byrne
 * 
 */
public class TokenTesting implements DatabaseUser
{
	...
	/**
	 * This takes a discovered context, and repeats the input transaction (where
	 * the token is submitted)
	 * and output transaction (where the token is observed in the result) with a
	 * new token. The new token could
	 * also be an attack string. The new output transaction is returned. The
	 * original input and output
	 * transaction can be the same object. The new transactions are not
	 * submitted for testing.
	 * 
	 * @param context
	 * @param newToken
	 * @return Element 0 is the new input transaction, element 1 is the new
	 *         output transaction. They may
	 *         be the same object, but both elements will be populated
	 * @throws UnrequestableTransaction
	 * @throws InterruptedScanException 
	 */
	public StandardHttpTransaction[] duplicateTokenTest(TokenContext context, String newToken,
			String stringSource, TransactionSource source,int testJobId) throws UnrequestableTransaction, InterruptedScanException
	{
		StandardHttpTransaction originalOutputTransaction =
				Scan.getInstance().getTransactionRecord().getTransaction(context.getOutputTransactionID());
		...
		if (originalInputTransaction.getId() == originalOutputTransaction.getId())
		{
			...
			MutableData newTestData = (MutableData) DataContainerUtils.resolveReferenceChain(newTestTransaction.getTransactionContainer(), context.getRequestDatum().getReferenceChain());
...
//			newTestTransaction.getRequestWrapper().setURI(uri, true);
		...
		else
		{
			...
			MutableData newTestData = (MutableData) DataContainerUtils.resolveReferenceChain(newInputTransaction.getTransactionContainer(), context.getRequestDatum().getReferenceChain());
			newTestData.setBytes(newToken.getBytes());
...
//			newInputTransaction.getRequestWrapper().setURI(uri, true);
	...
	public DiscoveredContexts findTokenContexts(StandardHttpTransaction transaction) throws InterruptedScanException
	{
	...
	public DiscoveredContexts findTokenContexts(StandardHttpTransaction transaction, String token) throws InterruptedScanException
	{
	...
	{
		return tokenPrefix
				+ StringUtils.generateRandomString(StringUtils.FORMAT_UPPER_CASE_ALPHA, tokenSuffixLength);
	}
	...
	{
		Log.debug("Initializing database for transaction storage");
		try
		{
			if (!database.tableExists(tableName))
			{
				String tableQuery =
						"CREATE TABLE " + tableName + " (token varchar(" + (tokenPrefixLength + tokenSuffixLength)
								+ "), " +
									"serialized_datum blob, PRIMARY KEY (token))";
		...
		{
			Log.fatal("Problem with creating database for token testing: " + e.toString(), e);
			System.exit(1);
		}
	}

	public void recordTokenTest(String token, Data datum)
	{
		String query = "INSERT INTO " + tableName + " "
				+ "(token, serialized_datum) "
				+ "VALUES (?, ?)";
		...
		{
			Log.error("Problem saving transaction: " + e.toString(), e);
		}
	}
	...
	/**
	 * Confirms that repeating the same requests with a different token will
	 * result in the same
	 * token context.
	 * 
	 * @param transaction
	 ...
	 */
	public boolean
			verifyTokenRepeatability(TokenContext originalContext, String stringSource, TransactionSource source, int testJobId)
					throws UnrequestableTransaction, InterruptedScanException
	{
		String newToken = generateToken();
		StandardHttpTransaction testTransactions[] =
	...
	private void addNodeResult(String token, Node node, int outputTransactionID, int originatingTransactionID, DiscoveredContexts results)
	{
		for (TokenContextType contextType : findNodeTokenContextTypes(token, node))
		{
			...
			StandardHttpTransaction transaction =
					Scan.getInstance().getTransactionRecord().getTransaction(outputTransactionID);
	...
	private Set<TokenContextType> findNodeTokenContextTypes(String token, Node node)
	{
	...
	private void parseByMimeType(StandardHttpTransaction transaction, DiscoveredContexts contexts)
	{
		...
		else
		{
			...
			while (m.find())
			{
				...
				else if (mimeType.equals("text/javascript") || mimeType.equals("text/ecmascript")
				...
				NonHtmlBodyContext context = new NonHtmlBodyContext(token, contextType, transaction.getId(), 
						originatingTransactionID, mimeType);
				contexts.addContext(context);
			}
		}
	}

	private void parseHeaders(StandardHttpTransaction transaction, DiscoveredContexts contexts)
	{
	...
	private void recursiveNodeSearch(Node node, DiscoveredContexts results, int outputTransactionID)
	{
		/*
		 * Add the node as a result if the node value contains the token. This
		 * should usually be for text nodes and attribute nodes
		 */
		if (node == null)
		{
			return;
...
//				break;
	...
	private Set<String> searchString(String targetString)
	{
	...
	public Data getTokenTest(String token)
	{
		try
		{
			return (Data) database.selectSimpleObject(
					"SELECT serialized_datum FROM " + tableName +
							" WHERE token = ?", new Object[] { token });
		}
		catch (DataNotFoundException e)
		{
			Log.warn("Token not found in database (" + token + "): " + e.toString());
			return null;
		}
		catch (Throwable e)
		{
			Log.error("Problem getting token from database (" + token + "): " + e.toString(), e);
		}
		return null;
	}
