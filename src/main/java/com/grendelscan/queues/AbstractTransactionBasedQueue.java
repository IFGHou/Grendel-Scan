...
public abstract class AbstractTransactionBasedQueue extends AbstractScanQueue
{
	...
	public synchronized QueueItem getNextQueueItem()
	{
		...
		{
			Log.error("Big problem with transaction db: " + e.toString(), e);
		}
		return null;
	}
	...
	{
		Log.debug("Initializing new database for " + getName() + " job storage");
		try
		{
	        String tableQuery = 
		...
		{
			Log.fatal("Problem with creating database for " + getName() + " queue: " + e.toString(), e);
			System.exit(1);
		}
	}
