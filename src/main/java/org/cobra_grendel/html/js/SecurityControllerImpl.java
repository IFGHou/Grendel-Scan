...
public class SecurityControllerImpl extends SecurityController
{
	private class LocalSecureClassLoader extends SecureClassLoader implements GeneratedClassLoader
	{
		...
		@Override public Class defineClass(String name, byte[] b)
		{
			return this.defineClass(name, b, 0, b.length, codesource);
		}
		
		@Override public void linkClass(Class clazz)
		{
	        ...
	        final Scriptable scope, final Scriptable thisObj, final Object[] args)
	{
		...
		else
		{
			...
			AccessControlContext acctx =
			        new AccessControlContext(new ProtectionDomain[] { (ProtectionDomain) securityDomain });
