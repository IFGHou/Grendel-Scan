...
class ElementFactory
{
	...
	public final HTMLElement createElement(HTMLDocumentImpl document, String name, int transactionId) throws DOMException
	{
		String normalName = name.toUpperCase();
		// No need to synchronize; read-only map at this point.
		HTMLElementBuilder builder = (HTMLElementBuilder) builders.get(normalName);
		if (builder == null)
		{
			// TODO: IE would assume name is html text here?
			HTMLElementImpl element = new HTMLElementImpl(name, transactionId);
			element.setOwnerDocument(document);
			return element;
		}
		else
		{
			return builder.create(document, name, transactionId);
		}
	}
}
