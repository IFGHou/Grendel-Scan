...
public class HTMLElementImpl extends ElementImpl implements HTMLElement
{
	...
	public HTMLElementImpl(String name, int transactionId)
	{
	...
	/*
	 * private volatile CSS2PropertiesImpl currentStyleDeclarationState; private
	 * volatile CSS2PropertiesImpl localStyleDeclarationState;
	 * 
	 ...
	 * null; } }
	 * 
	 * protected final void forgetStyle(boolean deep) { //TODO: Maybe this
	 * should redo style declarations in //CSS, not the whole properties object?
	 * synchronized(this) { this.currentStyleDeclarationState = null; if(deep) {
	 ...
	 */
	/*
	 * public CSS2PropertiesImpl getCurrentStyle() { CSS2PropertiesImpl sds;
	 * synchronized(this) { sds = this.currentStyleDeclarationState; if(sds !=
	 * null) { return sds; } } // Can't do the following in synchronized block
	 * (reverse locking order with document). // First, add declarations from
	 * stylesheet sds = this.addStyleSheetDeclarations(sds); // Now add local
	 * style if any. CSS2PropertiesImpl localStyle = this.getStyle(); if(sds ==
	 * null) { sds = new CSS2PropertiesImpl(this);
	 ...
	 * happen). CSS2PropertiesImpl setProps = this.currentStyleDeclarationState;
	 * if(setProps != null) { return setProps; }
	 * this.currentStyleDeclarationState = sds; return sds; } }
	 */
	// /**
	...
	// return sds;
	// }
	public HTMLElementImpl(String name, boolean noStyleSheet, int transactionId)
	{
		super(name, transactionId);
	 ...
	 */
	/*
	 * protected final CSS2PropertiesImpl
	 * addStyleSheetDeclarations(CSS2PropertiesImpl style) { Node pn =
	 * this.parentNode; if(pn == null) { // do later return style; } String
	 * classNames = this.getClassName(); if(classNames != null &&
	 ...
	 * this.findStyleDeclarations(elementName, id, className); if(sds != null) {
	 * Iterator sdsi = sds.iterator(); while(sdsi.hasNext()) {
	 * CSSStyleDeclaration sd = (CSSStyleDeclaration) sdsi.next(); if(style ==
	 * null) { style = new CSS2PropertiesImpl(this); }
	 * style.addStyleDeclaration(sd); } } } } else { String id = this.getId();
	 * String elementName = this.getTagName(); Collection sds =
	 * this.findStyleDeclarations(elementName, id, null); if(sds != null) {
	 * Iterator sdsi = sds.iterator(); while(sdsi.hasNext()) {
	 * CSSStyleDeclaration sd = (CSSStyleDeclaration) sdsi.next(); if(style ==
	 * null) { style = new CSS2PropertiesImpl(this); }
	 * style.addStyleDeclaration(sd); } } } return style; } protected final
	 * Collection findStyleDeclarations(String elementName, String id, String
	 * className) { HTMLDocumentImpl doc = (HTMLDocumentImpl) this.document;
	 * if(doc == null) { return null; } StyleSheetAggregator ssa =
	 * doc.getStyleSheetAggregator(); return ssa.getStyleDeclarations(this,
	 * elementName, id, className); }
	 */
	 ...
	 */
	/*
	 * public CSS2PropertiesImpl getStyle() { CSS2PropertiesImpl sds;
	 * synchronized(this) { sds = this.localStyleDeclarationState; if(sds !=
	 * null) { return sds; } sds = new CSS2PropertiesImpl(this); // Add any
	 ...
	 * null ? "" : " with ID '" + id + "'"; this.warn("Unable to parse style
	 * attribute value for element " + this.getTagName() + withId + ".", err); } }
	 * this.localStyleDeclarationState = sds; } // Synchronization note: Make
	 * sure getStyle() does not return multiple values. return sds; }
	 */
	// /**
	// * This method may return <code>null</code> if
	// * there's no style declaration applicable to
	 ...
	 * new StyleSheetRenderState(prevRenderState, this); } public int
	 * getOffsetTop() { CSS2PropertiesImpl style = this.getCurrentStyle();
	 * if(style == null) { return 0; } String topText = style.getTop(); return
	 * topText == null ? 0 : HtmlValues.getPixelSize(topText,
	 * this.getRenderState(), 0); }
	 * 
	 * public int getOffsetLeft() { CSS2PropertiesImpl style =
	 * this.getCurrentStyle(); if(style == null) { return 0; } String leftText =
	 * style.getLeft(); return leftText == null ? 0 :
	 * HtmlValues.getPixelSize(leftText, this.getRenderState(), 0); }
	 * 
	 * public int getOffsetWidth() { CSS2PropertiesImpl style =
	 * this.getCurrentStyle(); if(style == null) { return 0; } String valueText =
	 * style.getWidth(); return valueText == null ? 0 :
	 * HtmlValues.getPixelSize(valueText, this.getRenderState(), 0); }
	 * 
	 * public int getOffsetHeight() { CSS2PropertiesImpl style =
	 * this.getCurrentStyle(); if(style == null) { return 0; } String valueText =
	 * style.getHeight(); return valueText == null ? 0 :
	 * HtmlValues.getPixelSize(valueText, this.getRenderState(), 0); }
	 * 
	 * public CSS2PropertiesImpl getParentStyle() { Object parent =
