...
public class JavaFunctionObject extends ScriptableObject implements Function
{
	...
	@Override public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		if (method == null)
		{
			throw new EvaluatorException("No method matching " + className + " with "
		...
		for (int i = 0; i < numParams; i++)
		{
			...
			if (linfo)
			{
				logger.info("call(): For method=" + method.getName() + ": Converted arg=" + arg + " (type="
				        + getTypeName(arg) + ") into actualArg=" + actualArg + ". Type expected by method is "
		...
		{
			throw new WrappedException(new InvocationTargetException(ite.getCause(), "Unable to call " + className
		...
		catch (IllegalArgumentException iae)
		{
			...
			throw new WrappedException(new IllegalArgumentException("Unable to call " + className
