...
public class DataFormatUtils {
	...
	private final static Pattern HEX_UPPER_PATTERN = Pattern
			.compile("^(?:[A-F0-9]{2}){2,}$");
	private final static Pattern HEX_LOWER_PATTERN = Pattern
			.compile("^(?:[a-f0-9]{2}){2,}$");
	private final static Pattern HEX_LOWER_0X_PREFIX_PATTERN = Pattern
			.compile("^0[xX](?:[a-f0-9]{2})+$");
	private final static Pattern HEX_UPPER_0X_PREFIX_PATTERN = Pattern
			.compile("^0[xX](?:[A-F0-9]{2})+$");
	private final static Pattern HEX_LOWER_SLASH_X_PATTERN = Pattern
			.compile("^(?:\\\\[xX][a-f0-9]{2})+$");
	private final static Pattern HEX_UPPER_SLASH_X_PATTERN = Pattern
			.compile("^(?:\\\\[xX][A-F0-9]{2})+$");

	private final static Pattern URL_ENCODED_QUERY_PATTERN = Pattern
			.compile("^[^\\x00-\\x20\\x7f-\\xff\\{\\}\"'`^#\\|\\[\\]\\(\\)<>=&]+=[^\\x00-\\x20\\x7f-\\xff\\{\\}\"'`^#\\|\\[\\]\\(\\)<>]*$");

	private final static Pattern URL_ENCODED_BASE_PATTERN = Pattern
			.compile("^[\\x21-\\x7e]+$");
	private final static Pattern URL_ENCODED_ESCAPE_PATTERN = Pattern.compile(
			"%[a-f0-9]{2}", Pattern.CASE_INSENSITIVE);

	private final static Pattern BASE64_PATTERN = Pattern
			.compile("^[a-zA-Z0-9+/]+={0,2}$");
	private final static Pattern BASE64_PRETTY_PATTERN = Pattern
			.compile("^(?:[a-zA-Z0-9+/]{64,76}={0,2}\\x0d?\\x0a)" + // has to be
																	// at least
																	// one full
																	// line
					"[a-zA-Z0-9+/]{0,76}+={0,2}" + "\\x0d?\\x0a?$" // last line
																	// doesn't
																	// have to
																	// end with
																	// CRLF
			);
	private final static Pattern BASE64_PRETTY_LINE_PATTERN = Pattern
			.compile("^([a-zA-Z0-9+/]+={0,2})(\\x0d?\\x0a)");
	private final static Pattern BASE64_WEB_PATTERN = Pattern
			.compile("^[a-zA-Z0-90\\-_]+={0,2}$");

	private final static Pattern ALPHANUMERIC_PATTERN = Pattern
			.compile("^[0-9A-Za-z]+$");
	private final static Pattern NUMERIC_PATTERN = Pattern.compile("^[0-9]+$");

	private final static Pattern ASCII_TEXT_PATTERN = Pattern
			.compile("^[\\x09\\x0d\\x0a\\x20-\\x7e]+$");

	private static final float WORD_PERCENTAGE_THRESHOLD = 0.30F;

	private final static Pattern CONTAINS_NUMBERS_PATTERN = Pattern
			.compile("[0-9]");
	private final static Pattern CONTAINS_UPPERCASE_PATTERN = Pattern
			.compile("[A-Z]");
	private final static Pattern CONTAINS_LOWERCASE_PATTERN = Pattern
			.compile("[a-z]");

	private static WordList wordlist;

	public static byte[] decodeData(final byte[] data,
			final DataFormatType format) throws DataFormatException {
		if (!format.isEncodeable()) {
			throw new NotImplementedException(
					format.toString()
							+ " is not implemented for decoding; it is a basic data format.");
		}
		switch (format) {
		...
		case HEX_UPPER_SLASH_X:
			return HexEncodingUtils.decodeHex(HexEncodingUtils
					.simplifyHexFormat(data));
		...
		default:
			throw new NotImplementedException(format.toString()
					+ " is not implemented for decoding.");
		}
	}

	public static byte[] encodeData(final byte[] data,
			final DataFormatType format, DataFormatOptions options)
		...
		switch (format) {
		case HEX_LOWER_SIMPLE:
			return HexEncodingUtils.encodeHex(data,
					HexEncodingUtils.HEX_DIGITS_LOWER, false, false);
		case HEX_UPPER_SIMPLE:
			return HexEncodingUtils.encodeHex(data,
					HexEncodingUtils.HEX_DIGITS_UPPER, false, false);

		case HEX_LOWER_WITH_0X_PREFIX:
			return HexEncodingUtils.encodeHex(data,
					HexEncodingUtils.HEX_DIGITS_LOWER, true, false);
		case HEX_UPPER_WITH_0X_PREFIX:
			return HexEncodingUtils.encodeHex(data,
					HexEncodingUtils.HEX_DIGITS_UPPER, true, false);
		case HEX_LOWER_SLASH_X:
			return HexEncodingUtils.encodeHex(data,
					HexEncodingUtils.HEX_DIGITS_LOWER, false, true);
		case HEX_UPPER_SLASH_X:
			return HexEncodingUtils.encodeHex(data,
					HexEncodingUtils.HEX_DIGITS_UPPER, false, true);

		case BASE64:
			return Base64.encodeBase64(data);
		case BASE64_PRETTY:
			return new Base64(options.BASE64_LINE_LENGTH,
					options.BASE64_LINE_DELIMITER, false).encode(data);
		case BASE64_WEB:
	...
	public static DataFormat getDataFormat(final byte[] data,
			final String mimeType) throws DataFormatException {
		...
		if (data == null || data.length == 0) {
			throw new DataFormatException(
					"Null data and can't figure out the MIME type");
		}
		...
		if (NUMERIC_PATTERN.matcher(dataString).matches()) {
			...
			if (BASE64_PATTERN.matcher(dataString).matches()) {
				...
				if (m.find()) {
					...
					format.options.BASE64_LINE_DELIMITER = m.group(2)
							.getBytes();
				} else {
					throw new IllegalStateException(
							"Apparent bug in parsing Base64 pretty");
				}
	...
	public static void initialize(final WordList wordlist) {
		DataFormatUtils.wordlist = wordlist;
	}

	public static boolean potentialBase64(final String dataString,
			final float wordPercentage) {
	...
	private static String stripWords(final String data) {
		...
		for (String word : wordlist.getReadOnlyWordsSortedBySize()) {
