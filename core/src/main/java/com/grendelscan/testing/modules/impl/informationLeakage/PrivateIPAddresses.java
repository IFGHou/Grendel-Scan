...
public class PrivateIPAddresses extends AbstractTestModule implements ByHttpResponseCodeTest
{
    private Finding finding;
    private static final String FINDING_NAME = "private_ip_address_finding_number";
    private final Pattern privateIPRegEx;

    public PrivateIPAddresses()
    {
        privateIPRegEx = Pattern.compile("\\b((?:10(:?\\.\\d{1,3}){3})|(?:192\\.168(?:\\.\\d{1,3}){2})|(?:172\\.[1-3]\\d(?:\\.\\d{1,3}){2}))\\b");
    }
    ...
    private boolean isPrivateAddress(final String address)
    {
    ...
    private synchronized void logFinding(final String description, final String url)
    {
        try
        {
            if (finding == null)
        ...
        catch (DataNotFoundException e)
        {
            ...
            String longDescription = "Private IP addreses defined by RFC 1918 are not routable on the Internet, but are " + "frequently used on Internet-exposed devices and then changed by Network Address Translation (NAT).<br><br>"
                            + "Private IP addresses were found in the following location(s):<br>\n" + description + "<br>\n";

            String impact = "Knowning the private IP address of a server can help an attacker to craft better attacks against internal assets.";
            String recomendation = "For HTTP headers, consult the web or application server's " + "documentation to disable this behavior. For IP addresses in the " + "response body, modify the application's behavior to surpress the "
                            + "inclusion of private IP addresses.";

            finding = new Finding(null, "Module00008", FindingSeverity.INFO, url, title, shortDescription, longDescription, impact, recomendation, HtmlUtils.makeLink("http://www.ietf.org/rfc/rfc1918.txt"));
    ...
    public void testByHttpResponseCode(final int transactionID, final int testJobId) throws InterruptedScanException
    {
        ...
        for (Header header : transaction.getResponseWrapper().getHeaders().getReadOnlyHeaders())
        {
            ...
            if (matcher.find() && isPrivateAddress(matcher.group(1)))
            {
                ...
                logFinding(HtmlUtils.makeLink(transaction.getSavedUrl(), transaction.getRequestWrapper().getAbsoluteUriString()) + ": " + matcher.group(1) + " was found in the header named \"" + header.getName() + "\"",
        ...
        if (MimeUtils.isWebTextMimeType(transaction.getResponseWrapper().getHeaders().getMimeType()))
        {
            ...
            while (matcher.find())
            {
                ...
                {
                    transaction.writeToDisk();
                    logFinding(HtmlUtils.makeLink(transaction.getSavedUrl(), transaction.getRequestWrapper().getAbsoluteUriString()) + ": " + matcher.group(1) + " was found in the response body",
                                    URIStringUtils.getFileUri(transaction.getRequestWrapper().getAbsoluteUriString()));
                    break;
                }
            }
