...
public class ResponseSamples {
	...
	public ResponseSamples(final String name,
			final int baseTemplateTransactionId) {
		samples = new DatabaseBackedList<StandardHttpTransaction>(
	...
	public void addNewSample(final StandardHttpTransaction sample) {
		synchronized (samples) {
			if (baseTemplateTransactionId > 0) {
				StandardHttpTransaction base = Scan.getInstance()
						.getTransactionRecord()
						.getTransaction(baseTemplateTransactionId);
				if (HttpResponseScoreUtils.scoreResponseMatch(sample, base,
						100, true, true) >= threshold) {
					return; // transaction too similar to base
				}
			}
			boolean uniqueResponse = false;
			for (StandardHttpTransaction t2 : samples) {
				if (HttpResponseScoreUtils.scoreResponseMatch(sample, t2, 100,
						true, true) < threshold) {
					uniqueResponse = true;
					break;
				}
		...
		synchronized (samples) {
			if (!target.isSuccessfullExecution()) {
				throw new IllegalArgumentException(
						"Transaction must be executed to compare the response");
			}

			for (StandardHttpTransaction sample : samples) {
				if (HttpResponseScoreUtils.scoreResponseMatch(target, sample,
						100, Scan.getScanSettings().isParseHtmlDom(),
						followRedirects) >= threshold) {
					return true;
				}
			}
			return false;
