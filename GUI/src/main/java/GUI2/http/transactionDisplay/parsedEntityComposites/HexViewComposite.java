...
public class HexViewComposite extends GComposite implements ParsedEntityComposite {
	...
	public HexViewComposite(com.grendelscan.GUI.customControls.basic.GComposite parent, int style, boolean editable) 
	{
		super(parent, style);
	...
	private void initGUI() 
	{
		SWTResourceManager.registerResourceUser(this);
			...
			this.setLayout(thisLayout);
			{
				...
				hexTable.setFontf(SWTResourceManager.getFont("Courier New", 9, SWT.BOLD, false, false));
					...
					hexTable.addListener(SWT.MouseDoubleClick, new Listener()
					{
						...
						public void handleEvent(Event event)
						{
							...
							while (index < hexTable.getItemCount())
							{
								...
								final GTableItem item = hexTable.getItem(index);
								...
								for (int i = 1; i < 17; i++)
								{
									...
									if (rect.contains(pt) && item.getText(i) != "")
									{
										...
										text.setFontf(SWTResourceManager.getFont("Courier New", 9, SWT.BOLD, false, false));
	...
	String getRowText(GTableItem item)
	{
	...
	private void populateTable(byte[] data)
	{
		for (int index = 0; index < data.length; index += 16)
		{
			...
			item.setFontf(SWTResourceManager.getFont("Courier New", 9, SWT.BOLD, false, false));
	...
	public byte[] getBytes()
	{
		...
		int index = 0;
		for (GTableItem item: hexTable.getItems())
		{
