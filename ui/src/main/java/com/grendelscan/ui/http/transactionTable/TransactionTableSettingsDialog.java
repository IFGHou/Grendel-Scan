 ...
 */
public class TransactionTableSettingsDialog extends com.grendelscan.ui.customControls.basic.GDialog
{
    ...
    public TransactionTableSettingsDialog(final GShell parent, final int style)
    {
        super(parent, style);
    }

    private boolean badPattern(final String pattern)
    {
        boolean bad = false;
        try
        {
    ...
    private void changeAllSources(final boolean selected)
    {
    ...
    private void displayFilter(final TransactionTableFilter filter)
    {
        if (filter != null)
        {
    ...
    private void enableNonTailSettings(final boolean enabled)
    {
    ...
    private void okButtonWidgetSelected(final SelectionEvent evt)
    {
        String message = "";
        ...
        {
            if (!(existingSettings.getHostPattern().equals(hostRegexText.getText()) && existingSettings.getPathPattern().equals(pathRegexText.getText()) && existingSettings.getQueryPattern().equals(queryRegexText.getText())
                            && existingSettings.getResponseCodePattern().equals(responseCodeRegexText.getText())

                            && authenticationSourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.AUTHENTICATION)
                            && overrideSourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.AUTOMATIC_RESPONSE_CODE_OVERRIDES) && baseUrlCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.BASE)
                            && categorizerSourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.CATEGORIZER) && cobraSourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.COBRA)
                            && enumerationSourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.ENUMERATION) && fuzzerSourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.FUZZER)
                            && manualRequestCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.MANUAL_REQUEST) && niktoSourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.NIKTO)
                            && proxySourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.PROXY) && spiderSourceCheckbox.getSelection() == existingSettings.containsSource(TransactionSource.SPIDER) && testSourceCheckbox
                                .getSelection() == existingSettings.containsSource(TransactionSource.MISC_TEST)))
            {
                changed = true;
    ...
    public boolean open(final TransactionTableFilter existingSettings)
    {
