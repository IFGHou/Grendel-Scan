...
import com.grendelscan.ui.http.transactionDisplay.parsedEntityComposites.AmfViewComposite;
import com.grendelscan.ui.http.transactionDisplay.parsedEntityComposites.BrowserComposite;
import com.grendelscan.ui.http.transactionDisplay.parsedEntityComposites.HexViewComposite;
import com.grendelscan.ui.http.transactionDisplay.parsedEntityComposites.ParsedEntityComposite;
import com.grendelscan.ui.http.transactionDisplay.parsedEntityComposites.RawBodyText;
 ...
 */
public class ParsedBodyComposite extends com.grendelscan.ui.customControls.basic.GComposite
{
    private enum View
    {
        RAW, HEX, CUSTOM
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ParsedBodyComposite.class);
    ...
    public ParsedBodyComposite(final GComposite parent, final int style, final boolean editable)
    {
        super(parent, style);
    ...
    protected void changeButtonSelection(final GButton button, final boolean useActiveControl)
    {
        ...
        catch (UpdateCanceledException e)
        {
            ...
            {
                LOGGER.warn("Not sure how to handle a null button here");
            }
        }
    }
    ...
    public void clearData()
    {
        ...
        {
            LOGGER.error("Not sure how this was reached", e);
        }
    }

    protected void disposeWidget(final Widget target)
    {
        if (target != null)
        {
            target.dispose();
    ...
    protected void initGUI()
    {
        ...
        int currentLeft = 0;
        {
...

            {
                ...
                hexEncodedRadioButton.addSelectionListener(new SelectionAdapter()
                {
                    @Override
            ...
            }
            {
                ...
                rawEncodedRadio.addSelectionListener(new SelectionAdapter()
                {
                    @Override
                    public void widgetSelected(final SelectionEvent event)
                    {
    ...
    protected void processButtonSelection(final SelectionEvent event)
    {
    ...
    protected void updateData(@SuppressWarnings("hiding") final byte[] body, @SuppressWarnings("hiding") final String mimeType)
    {
        if (!this.body.equals(body) || this.mimeType != mimeType)
        {
            this.mimeType = mimeType;
            contentCategory = HttpContentTypeCategory.getContentType(mimeType);
            this.body = body;
            updateEncodingOptions();
