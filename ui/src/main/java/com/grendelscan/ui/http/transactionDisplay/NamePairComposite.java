...
public class NamePairComposite extends GComposite
{
    ...
    public NamePairComposite(final com.grendelscan.ui.customControls.basic.GComposite parent, final int style, final int initialHeight, final boolean editable)
    {
        super(parent, style);
    ...
    public String getFirstValue(final String name, final boolean caseSensative)
    {
        for (GTableItem item : pairsTable.getItems())
        {
            if (!caseSensative && item.getText(0).equalsIgnoreCase(name) || caseSensative && item.getText(0).equals(name))
            {
    ...
    protected void initGUI()
    {
        ...
        }
        {
            ...
            if (editable)
            {
                pairsTable.addListener(SWT.MouseDoubleClick, new Listener()
                {
                    @Override
