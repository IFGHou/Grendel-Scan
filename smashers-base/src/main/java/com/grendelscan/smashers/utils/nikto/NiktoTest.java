...
public class NiktoTest
{
    ...
    private final String matchAndPattern;
    private final String matchAntiPattern1;
    private final String matchAntiPattern2;
    private final String matchOrPattern;
    ...
    /*
     * 0 "000001", - test number 1 "0", - osvdb 2 "b", - test categories 3 "/TiVoConnect?Command=QueryServer", - url 4 "GET", - method 5 "Calypso
     * Server", - match pattern 6 "", - match or 7 "", - match and 8 "", - anti match pattern 1 9 "", - anti match pattern 1 10 "The Tivo Calypso server is running. This page will display the version
     * and platform it is running on. Other URLs may allow download of media.", 11 "", - data 12 "" - headers
     */

    public NiktoTest(final String[] line) throws ParseException
    {
        if (line.length != 13)
        {
    ...
    /**
     * Checks with index.php have some problems with false positives. This emulates Nikto's logic for dealing with it.
     * 
     * @param transaction
     * @return
     * @throws InterruptedScanException
     */
    private boolean checkIndexPHP(final StandardHttpTransaction transaction, final String baseUri, final int testJobId) throws InterruptedScanException
    {
        boolean okay = true;
    ...
    public Set<String> constructTestUris(final String baseUri, final int testJobId) throws InterruptedScanException
    {
        String uriPrefix;
        if (baseUri.endsWith("/"))
        {
    ...
    /**
     * There are several url bugs in the Nikto test database. This will try to fix them.
     * 
     * @param url
     * @return
     */
    private String fixUrlBugs(final String url)
    {
        String newUrl = url;
        ...
        /*
         * There are several tests that have two forward slashes by mistake. That won't cause a problem for most web servers, but it does cause a problem for Java.net.URI.
         * 
         * Until I create a completly independent HTTP handler for StandardHttpTransaction, this needs to be fixed.
         */
    ...
    public boolean matches(final StandardHttpTransaction transaction, final String baseUri, final int testJobId) throws NumberFormatException, InterruptedScanException
    {
        boolean matches = false;
        if (transaction.getLogicalResponseCodeWithoutProfileGeneration() != 404 && (matchesPattern(transaction, matchPattern) || !matchOrPattern.equals("") && matchesPattern(transaction, matchOrPattern)) && matchesPattern(transaction, matchAndPattern) && !( // Check
                                                                                                                                                                                                                                                                  // the
                                                                                                                                                                                                                                                                  // anti
                                                                                                                                                                                                                                                                  // patterns
                                                                                                                                                                                                                                                                  // if
                                                                                                                                                                                                                                                                  // they
                                                                                                                                                                                                                                                                  // exist
                        !matchAntiPattern1.equals("") && matchesPattern(transaction, matchAntiPattern1) || !matchAntiPattern2.equals("") && matchesPattern(transaction, matchAntiPattern2)) && checkIndexPHP(transaction, baseUri, testJobId))
        {
            matches = true;
        }
    ...
    private boolean matchesPattern(final StandardHttpTransaction transaction, final String pattern) throws NumberFormatException, InterruptedScanException
    {
        boolean matches = false;
