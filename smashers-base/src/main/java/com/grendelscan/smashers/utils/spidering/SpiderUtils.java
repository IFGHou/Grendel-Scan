 ...
 * 
 */
package com.grendelscan.smashers.utils.spidering;
...
public class SpiderUtils implements DatabaseUser
{
	...
	public void initializeDatabase()
	{
		...
		{
			LOGGER.error("Problem with creating database for spider utils: " + e.toString(), e);
			System.exit(1);
		}
	}
		...
		if (Scan.getScanSettings().getUrlFilters().isUriAllowed(url))
		{
			...
			else
			{
				...
				for (NameValuePair param : params)
				{
					...
					{
						LOGGER.error("Error with getting cound of URLs, which shouldn't happen: ", e);
	...
	private boolean isUrlHashNew(String urlhash)
	{
		...
		{
			LOGGER.error("Error with getting could of URLs, which shouldn't happen: ", e);
			throw new IllegalStateException(e);
		}
		catch (Throwable e)
		{
