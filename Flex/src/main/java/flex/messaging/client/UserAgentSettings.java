...
public class UserAgentSettings
{
     ...
     * This lets us sniff for all IE versions.
     */
     ...
     * This lets us sniff for all Firefox versions.
     */
    ...
    /**
     * The default number of streaming connections per session. In IE, there can 
     * be 4 persistent HTTP connections per session. In Firefox, this number is 
     * determined by network.http.max-connections-per-server config which is 8 
     * by default. Streaming endpoints can technically support 
     * one less than this number of streaming connections per session (3 for IE,
     * 7 for Firefox) but by default, we will have one streaming connection per
     * session which can be changed by increasing maxStreamingConnectionsPerSession
     * property if needed.  
     */
